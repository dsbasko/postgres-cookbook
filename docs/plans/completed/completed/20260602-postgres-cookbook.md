...
- [x] 08-01 window-basics-partition-order — sqlc-юнит на лабораторной purchases_lab (7 покупок, детерминированный seed). Контраст «агрегат vs окно»: GROUP BY схлопывает покупки клиента в одну строку (CustomerTotals), а `sum(cents) OVER (PARTITION BY customer)` оставляет все 7 строк и доклеивает итог клиента + общий итог `OVER ()` (WindowTotals). Кульминация — `sum(...) OVER (PARTITION BY customer ORDER BY day, id)` = running total (накопление обнуляется на границе клиента, id — tie-break внутри дня). Все sum приведены к `::bigint` → Go int64. Заборчик: арифметика по клиенту сходится с GROUP BY (Алиса 1270, Борис 730, Карина 780, общий 2780), но покупки не потеряны
- [x] 08-02 ranking-and-top-n-per-group — sqlc-юнит на drink_sales_lab. Три ранг-функции рядом в одной партиции (ORDER BY units DESC): `row_number` уникален (1,2,3,4), `rank` ставит ничью 120/120 как 2,2 и прыгает на 4, `dense_rank` идёт 2,2,3 — ничья «Капучино/Эспрессо» показывает разницу наглядно. Top-N на группу: CTE с `row_number() OVER (PARTITION BY category ORDER BY units DESC) = 1` → лидер каждой категории. `ntile(4)` режет продажи на квартили (корзина 1 — лидеры). Заборчик: для top-1 на группу row_number()=1 портируемее, чем DISTINCT ON (04-04)
- [x] 08-03 lag-lead-and-frames — sqlc-юнит на daily_revenue_lab (выручка по дням, дыра 05.02 НАМЕРЕННО). `lag/lead` день-к-дню с `coalesce(...::text,'—')` для краёв (нет вчера/завтра) + дельта. Гвоздь урока — `ROWS BETWEEN 2 PRECEDING AND CURRENT ROW` (3 физические строки) vs `RANGE` по дате: после пропуска 05.02 они РАСХОДЯТСЯ (ma_rows берёт 3 строки подряд, ma_range — только даты в окне) — наглядно, почему RANGE считает по значению, а не по позиции. Скользящие средние округлены `::numeric(.,2)`. Заборчик: ROWS — позиция, RANGE — значение; пропуски в ряду делают их разными
- [x] 08-04 recursive-ctes — escape-hatch (psql `demo.sql`, без go.mod, не в go.work). ⚠️ ПРИЧИНА escape-hatch: sqlc v1.30.0 НЕ парсит SQL-стандартную секцию `CYCLE` (виртуальные колонки is_cycle/path вне схемы — «column is_cycle does not exist»), а урок ровно про неё. Часть 1: `WITH RECURSIVE` спускается по category_tree_lab (дерево меню Brew) с глубиной/путём, `ORDER BY idpath` (массив id) → детерминированный pre-order без зависимости от collation. Часть 2: на зациклённом cyclic_routes_lab (Склад→Цех→Кафе→Склад) `CYCLE id SET is_cycle USING path` ловит повтор и ОСТАНАВЛИВАЕТ рекурсию (is_cycle=t на 4-м шаге). Лабораторные столы DROP+CREATE, канон не тронут. Заборчик: без CYCLE/LIMIT рекурсия по графу не завершится
- [x] 08-05 lateral-joins — sqlc-юнит. ⭐ убийца N+1: `LEFT JOIN LATERAL (SELECT ... FROM orders_lab WHERE customer_id = c.id ORDER BY amount DESC LIMIT 3) ON true` — top-3 заказа на клиента ОДНИМ запросом вместо «1 запрос на клиентов + N запросов на заказы». LEFT JOIN LATERAL сохраняет Карину без заказов ('—'); у Алисы 4 заказа, top-3 отсекли самый дешёвый. Второй пример — LATERAL c LIMIT 1 (самый крупный заказ на клиента). Заборчик: LATERAL видит колонки левой таблицы (обычный подзапрос в FROM — нет), идеален для top-N-на-группу и «развернуть связанное»
- [x] 08-06 grouping-sets-rollup-cube — sqlc-юнит на sales_cube_lab (магазин × категория). `ROLLUP (shop, category)` → листья + подытог по магазину + общий итог (иерархия слева направо); `CUBE (shop, category)` добавляет подытоги по категории по всей сети (все комбинации); `GROUPING SETS ((shop),(category),())` — только нужные срезы. `grouping()` даёт «уровень» агрегации, NULL подытогов заменён на «— все —» через CASE. Один проход вместо UNION ALL нескольких GROUP BY. Заборчик: grouping() отличает «настоящий NULL» от «строки итога»; rollup для иерархий, cube — осторожно (2^N групп)
- [x] verification-gate; course.yaml; web-check-coverage — всё зелёное: `cd lectures && go work sync` exit 0; `make build` собрал все workspace-модули (5 sqlc-юнитов 08-01/02/03/05/06; escape-hatch 08-04 без go.mod в workspace не входит); `gofmt -l` пусто и `go vet` чисто по всем 5; `go test ./internal/...` зелёный против живой PG18; `make gen` воспроизводим у всех 5 sqlc-юнитов (хэш internal/db бит-в-бит совпал при повторе); `make db-reset` идемпотентен (двойной прогон, лабораторные столы DROP+CREATE); `make run` детерминирован (повторный прогон бит-в-бит) и БИТ-В-БИТ совпадает с вставленным в README выводом у всех 6 (даты/суммы фиксированы seed'ом, ранги/средние округлены, путь сортируется по массиву id); оба языка (ru+en) + стаб README.md у всех 6. course.yaml: модуль 08 раскомментирован, 6 lessons добавлены (parking-комментарий → «09–10»), go.work получил 5 sqlc-юнитов; `make web-check-coverage` зелёный (52 lessons, RU 52/52, EN 52/52, 0 mismatches); бонусом `make web-build` собрал статический экспорт — все 12 страниц (6 юнитов × ru/en) рендерятся, `web/out/404.html` на месте. Стенд снесён `down -v`
...
- [x] 09-01 merge-and-copy — escape-hatch raw-pgx (COPY FROM STDIN = `pool.CopyFrom`, а `MERGE ... RETURNING merge_action()` парсер sqlc v1.30.0 не понимает). COPY заливает поставку в стейджинг `supplier_feed_lab`; один MERGE сверяет её со складом `stock_lab` — все три ветки (WHEN MATCHED AND on_hand=0 → DELETE / MATCHED → UPDATE / NOT MATCHED → INSERT), `merge_action()` в RETURNING отчитывается по строкам (в DELETE-ветке `t.*` — значения удалённой строки). Порядок RETURNING у MERGE не определён → сортировка по SKU в Go. Заборчик: MERGE НЕ race-safe для upsert (для конкурентного — `ON CONFLICT` из 03-04), COPY лить в стейджинг, а не в боевую таблицу. Вывод детерминирован
- [x] 09-02 skip-locked-job-queue — escape-hatch raw-pgx, Go-центричный (конкурентность: N горутин-воркеров, у каждого своя tx). `jobs_lab`, 12 задач, 4 воркера; каждый в цикле `SELECT ... FOR UPDATE SKIP LOCKED LIMIT 1` → `UPDATE done` → COMMIT, на `pgx.ErrNoRows` выходит. Распределение по воркерам недетерминированное by design → печатаем ИНВАРИАНТЫ (забрано 12, уникальных 12, дублей 0, в БД done 12 / queued 0), они стабильны от прогона к прогону. ⚠️ многооператорный DDL с аргументом `$1` падал 42601 (extended-протокол не принимает несколько команд в строке) → DROP+CREATE без аргументов и параметризованный INSERT разнесены на два `Exec`. Заборчик: короткая транзакция воркера, SKIP LOCKED жертвует строгим порядком, таблица-очередь — решение «до брокера»
- [x] 09-03 transactional-outbox — канонический sqlc-юнит на КАНОНИЧЕСКИХ `orders`/`outbox` (та пара, что едет в capstone 10-05 → CDC-эстафета kafka-cookbook; `schema.sql` пуст — только канон). query.sql: InsertOrder/InsertOutbox (атомарная пара заказ+событие в одной tx через `db.New(tx)`), ClaimUnpublished (relay под `FOR UPDATE SKIP LOCKED` по partial-индексу `outbox_unpublished_idx`), MarkPublished, Count*. Именованные параметры (`@amount::numeric`) → аккуратные поля структуры; amount через `pgtype.Numeric.Scan`. Демо: атомарная запись 3 заказов, откатившаяся tx не оставляет ни заказа, ни события (счётчики 3/3), relay вычитывает и доставляет. Детерминизм: `TRUNCATE order_items, orders, outbox RESTART IDENTITY CASCADE` в начале run → id с 1; payload нормализован jsonb (`{"amount": ..., "order_id": ...}`). `make gen` воспроизводим (хэш internal/db бит-в-бит совпал при повторе). Заборчик: at-least-once + идемпотентность consumer'а (`processed_outbox_ids`), high-churn bloat, развилка relay-руками vs CDC (10-05)
- [x] 09-04 listen-notify — escape-hatch raw-pgx (LISTEN/NOTIFY живут на уровне `conn.WaitForNotification`, в sqlc метода нет). Триггер `AFTER INSERT` → `pg_notify('brew_events', json_build_object(...))`; слушатель на ВЫДЕЛЕННОМ `pool.Acquire`-коннекте. Обе оговорки детерминированно через ожидание-с-таймаутом (400ms): транзакционность (NOTIFY придержан до COMMIT — до коммита таймаут, после — payload `{"id" : 1, "name" : "Эспрессо"}`) и at-most-once (UNLISTEN → INSERT 'Латте' в пустоту → re-LISTEN → таймаут, NOTIFY не хранится). Заборчик: лимит payload 8 КБ (слать id, не тело), несовместимость с транзакционным пулингом (PgBouncer → 10-04), NOTIFY=«проснись» поверх надёжного outbox
- [x] 09-05 triggers-and-volatility — escape-hatch psql (серверный DDL/PL-pgSQL, sqlc неприменим; финал модуля). demo.sql: BEFORE-триггер автозаполняет `updated_at` на UPDATE (печатаем булев `updated_at_bumped`, не `now()`); AFTER-триггер по `TG_OP` пишет аудит OLD/NEW для INSERT/UPDATE/DELETE (асимметрия `∅`: нет OLD в INSERT, нет NEW в DELETE); волатильность — `provolatile` i/s/v из `pg_proc` + практическое следствие: индекс по IMMUTABLE собран, по VOLATILE отбит **42P17**. ⚠️ `f_vol_int` намеренно `plpgsql`: тривиальную `sql`-функцию планировщик встроил бы в выражение и метка VOLATILE потерялась бы (индекс собрался → SQLSTATE 00000), а `plpgsql` не встраивается → метка в силе → 42P17. Явная секция «когда НЕ класть логику в БД» (триггеры — для инвариантов данных, бизнес-логика — в приложении; честность метки волатильности). stdout детерминирован (сырой текст ошибки 42P17 — в stderr)
- [x] verification-gate; course.yaml; web-check-coverage — всё зелёное: `cd lectures && go work sync` exit 0; `make build` собрал все workspace-модули (09-01/02/03/04 — go.mod-юниты; escape-hatch 09-05 без go.mod в workspace не входит); `gofmt -l` пусто и `go vet` чисто по 4 Go-юнитам; `go test ./internal/...` зелёный против живой PG18; `make gen` воспроизводим у 09-03 (хэш internal/db бит-в-бит совпал при повторе); `make run` детерминирован (повторный прогон бит-в-бит) и БИТ-В-БИТ совпадает с вставленным в README выводом у всех 5 юнитов (проверено программно по ru И en; 09-02 — инварианты вместо недетерминированной раскладки воркеров; 09-05 — stdout, сырая ошибка в stderr); оба языка (ru+en) + стаб README.md у всех 5. course.yaml: модуль 09 раскомментирован, 5 lessons добавлены (модуль 10 остаётся запаркован в комментарии), go.work получил 4 Go-юнита; `make web-check-coverage` зелёный (57 lessons, RU 57/57, EN 57/57, 0 mismatches); бонусом `make web-build` собрал статический экспорт — все 10 страниц (5 юнитов × ru/en) рендерятся, `web/out/404.html` на месте. Стенд снесён `down -v`
...
- [x] 10-01 build-the-brew-schema-capstone — raw-pgx Go-капстон (go.mod, в go.work). Лабораторная подсистема лояльности (cap_members/cap_ledger) проходит весь курс: типы (деньги BIGINT-центы), ограничения PK/UNIQUE/CHECK/FK (каждый отказ печатает SQLSTATE — 23505/23514/23514/23503), CRUD с INSERT ... RETURNING, индекс, проверенный планом (`Seq Scan` до CREATE INDEX → `Index Scan` после, узел из EXPLAIN FORMAT JSON), и транзакция SERIALIZABLE с ретраем на 40001 (детерминированная инъекция «ночного начисления» как в 05-05 → попытка 1 падает, попытка 2 успешна, баланс 15.00+1.00+5.00=21.00). Integration-тест `cmd/demo/main_test.go` (4 теста, skip-when-down): схема+CRUD, отказы ограничений, смена плана индексом, выживание ретрая
- [x] 10-02 price-and-promo-engine — raw-pgx Go-капстон. PG18 temporal `PRIMARY KEY (drink_id, valid WITHOUT OVERLAPS)` на price_periods (смежные периоды ок, пересечение → 23P01); классический `EXCLUDE USING gist (code WITH =, span WITH &&)` на promo_windows (тот же запрет до PG18; другой код с тем же окном проходит); PG18 `RETURNING old.price_cents, new.price_cents` наполняет price_audit без отдельного SELECT (3.20→3.40). btree_gist через CREATE EXTENSION. Escape-hatch: sqlc v1.30.0 не парсит WITHOUT OVERLAPS и RETURNING old/new. Тест (3, skip-when-down): отбой пересечения temporal/EXCLUDE, захват old/new
- [x] 10-03 app-anti-patterns-clinic — raw-pgx Go-капстон. Пять болезней с лечением и ИЗМЕРЕНИЕМ: N+1 (1+3=4 round-trip'а) → батч `= ANY($1::text[])` (1) на каноне customers/orders; SELECT * (9 столбцов) → явные (2); non-sargable `lower(email)=` (Seq Scan) → expression-индекс (Index Scan); глубокий OFFSET 40000 (сканер прочитал 40010 строк, замер через EXPLAIN ANALYZE actual rows листа) → keyset `WHERE id>40000` (10); огромный IN (1000 литералов) → `= ANY($1::bigint[])` (1 параметр). Лабораторные events_lab/accounts_lab по 50k (generate_series). Тест (4, skip-when-down): keyset<offset rows, expression-индекс, ANY=IN, ширина SELECT *
- [x] 10-04 pooling-from-the-app — raw-pgx Go-капстон. Транзакционный пул (pgbouncer) держит бэкенд на ОДНУ транзакцию, не на сессию → ломается session-state. Три поломки+фикса (симуляция на чистом PG разводом по бэкендам пула, WithMaxConns(4)): session advisory-лок (A держит lock(42); B try→false, B unlock→false+WARNING в stderr → лок течёт) → `pg_advisory_xact_lock` (снимается на COMMIT: держится в tx true, после false); LISTEN/NOTIFY (выделенный коннект получает; коннект без LISTEN — таймаут) → выделенный коннект (как 09-04); prepared statements → `QueryExecModeSimpleProtocol` через pg.Option. Тест (4, skip-when-down)
- [x] 10-05 the-cdc-seam-handoff — raw-pgx Go-капстон, ФИНАЛ курса. `db/init.sql` БАЙТ-совместим с kafka-cookbook (DDL дословно идентичен, verified diff игнорируя комментарии); демо применяет его (idempotent), показывает REPLICA IDENTITY FULL на drinks/articles/customers, `CREATE PUBLICATION dbz_publication FOR TABLE ...`, и ДОКАЗЫВАЕТ шов логическим декодированием (временный слот test_decoding → UPDATE → before-image old-key содержит все 9 столбцов = REPLICA IDENTITY FULL работает; без FULL был бы только id). Требует wal_level=logical (уже в docker-compose). Тест (4: DB-free байт-compat гард `TestInitSQL_ByteCompatTokens` + 3 интеграционных skip-when-down: публикация=ровно 3 таблицы, replident='f', before-image=9 столбцов)
- [x] verification-gate + `go test` по капстонам; course.yaml; web-check-coverage — всё зелёное: `cd lectures && go work sync` exit 0; `make build` собрал все workspace-модули (5 новых Go-капстонов — go.mod-юниты, все в go.work); `gofmt -l` пусто и `go vet` чисто по всем 5; `go test ./internal/...` зелёный против живой PG18 + все 5 капстон-тестов зелёные (skip-when-down оставляет `go test` зелёным без стенда); `make run` детерминирован (повторный прогон бит-в-бит) и БИТ-В-БИТ совпадает с вставленным в README выводом у всех 5 (verified: captured output — verbatim contiguous substring обоих README ru+en; недетерминированное — xid слота, pid бэкенда, now() — не печатается); оба языка (ru+en) + стаб README.md у всех 5. course.yaml: модуль 10 раскомментирован, 5 lessons добавлены (parking-комментарий убран — все 11 модулей объявлены); `make web-check-coverage` зелёный (62 lessons, RU 62/62, EN 62/62, 0 mismatches); `make web-build` собрал статический экспорт — все 10 страниц (5 юнитов × ru/en) рендерятся, `web/out/404.html` на месте. Стенд снесён `down -v`

---

### Task 4.1: Verify acceptance criteria

- [x] все 11 модулей объявлены в course.yaml, lessons совпадают с ФС — `make web-check-coverage` зелёный (62 lessons, RU 62/62, EN 62/62, 0 mismatches; `grep -c '- id:'` = 11 модулей)
- [x] каждый опубликованный юнит имеет ОБА языка — RU 62/62, EN 62/62 в выводе web-check-coverage (нет noindex-fallback)
- [x] `make web-build` — полный статический экспорт без ошибок (exit 0, 151 HTML-страниц, `web/out/404.html` присутствует; postbuild fix-static-images отработал)
- [x] `make build` (workspace-loop по `go list -m`) зелёный по всем 46 модулям (exit 0, 0 ошибок); `go test ./...` зелёный по всем модулям против живой PG18 (internal/pg, internal/brew + капстоны — интеграционные тесты реально выполнились, не skip: TestReset_AppliesCanonAndIsIdempotent, TestApply_ExtraDDL, TestNewPool_Sandbox — PASS). Прим.: `go build ./...` из корня workspace не работает (multi-module) — канонический путь `make build`
- [x] выборочно `make lecture L=<...>` (дефолт — `run`) для 3 типов: sqlc (00-04-typed-queries-with-sqlc), escape-hatch (06-01-reading-explain-analyze-buffers), капстон (10-01-build-the-brew-schema-capstone) — вывод каждого БИТ-В-БИТ совпал со вставленным в `i18n/ru/README.md`; у escape-hatch 06-01 `run` — алиас на `demo.sql` через psql (verified в Makefile)
- [x] scope-дисциплина: нет DBA/DevOps-тем (0 `##`-заголовков про репликацию-setup/backup/pg_dump/тюнинг/failover/standby/k8s; «репликация» встречается только как in-scope CDC-шов 10-05); production-fence применён в 61/62 юнитах (`Заборчик`/`Что мы упростили`/«в проде…»/«в реальном…»); единственное исключение — 00-01 (чистый orientation-юнит про клиент-сервер+песочницу, без упрощения, требующего забора)

### Task 4.2: [Final] Документация и кросс-связка

- [x] обновлён `README.md`: TOC в `## Table of contents` перегенерён `make web-generate-readme-toc` (11 модулей / 62 урока, EN), embed между маркерами БИТ-В-БИТ совпадает с выводом генератора (verified Python-диффом — diff-stable). RU-вариант `TOC_LANG=ru` проверен — генерится корректно (11 модулей, ссылки на `i18n/ru/`); отдельного RU-README нет — единый EN-README по конвенции sibling-репо kafka-cookbook (RU-контент живёт в `i18n/ru/` и на сайте через переключатель)
- [x] кросс-ссылки: postgres→kafka уже в `README.md` (раздел «A shared universe with kafka-cookbook» — описан handoff 10-05: REPLICA IDENTITY FULL → CREATE PUBLICATION dbz_publication → Debezium без переписывания схемы). Реципрокная ссылка kafka→postgres добавлена в локальный `kafka-cookbook/README.md` (абзац про sibling postgres-cookbook + CDC-эстафету) — НЕ закоммичена в том репо: kafka-cookbook — отдельный опубликованный репозиторий со своим pipeline private→public (repo-sync); оставлена рабочей правкой на ревью пользователю, чтобы не разойтись с source-of-truth (private-репо локально отсутствует)
- [x] `CLAUDE.md` дополнен паттернами авторинга: список парсер-ограничений sqlc v1.30.0 (RETURNING old/new, GENERATED VIRTUAL, WITHOUT OVERLAPS, MERGE merge_action, CYCLE, multi-array unnest, LISTEN/NOTIFY/CopyFrom) как триггеры escape-hatch; escape-hatch `db-reset` через psql; секция «Determinism (so make run matches the README)» (не печатать now()/uuid/xid/pid, SQLSTATE вместо текста, reseed, EXPLAIN-флаги, инварианты для конкурентных демо); секция «Authoring patterns» (lab-таблицы `*_lab`, per-unit DDL через runtime.Caller, CREATE EXTENSION, форма капстонов с skip-when-down тестами, gotchas: 42601 multi-statement+args, sql-vs-plpgsql inlining волатильности, go vet `%`-литерал, TOC-truncation по первой запятой)
- [x] план перемещён в `docs/plans/completed/` (`git mv`). Курс готов целиком: все 11 модулей, 62 урока, фазы 0–4 завершены (Bootstrap shell, Shared foundation, Reference units, Authoring всех модулей, Verify acceptance). Все секции плана [x]; остаётся только Post-Completion (ручное/внешнее — без чекбоксов)
