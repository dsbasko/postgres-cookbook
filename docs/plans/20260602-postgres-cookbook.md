...
- [ ] `git init` в корне; создать `.gitignore` (node_modules, .next, out, web/public/static/lectures кэш, *.tsbuildinfo). ⚠️ **НЕ копировать `.gitignore` дословно:** эталон содержит `/docs/` (строка 53) — он бы заигнорил ЭТОТ план; обязательно убрать `/docs/` (или добавить `!docs/plans/`). Убрать Kafka-only строки: `connect-plugins/*`, все `lectures/*/*/<kafka-бинарь>` (producer/consumer/courier/…); добавить вместо них `lectures/*/*/demo` (имя нашего бинаря `cmd/demo`)
- [ ] скопировать `web/` дословно (включая `.eslintrc.json` + `.prettierrc` — нужны для `web-lint`); в `web/package.json` поменять только `name` → `postgres-cookbook-web` (зависимости оставить как есть; это сохраняет точный пин `@dsbasko/cookbook-engine@1.0.0`)
- [ ] скопировать `.github/workflows/deploy.yml` без изменений
- [ ] создать корневой `package.json` (`name: postgres-cookbook`, workspace-маркер) и `pnpm-workspace.yaml` (`packages: [web]`)
- [ ] создать корневой `Makefile`: перенести цели `web-install/web-dev/web-build/web-lint/web-typecheck/web-clean/web-check-coverage/web-generate-readme-toc` дословно; НЕ переносить `connect-*` и `DEBEZIUM/CLICKHOUSE/ES_VERSION`
- [ ] verification: `git status` чистый от игнор-файлов; структура `web/app` содержит все 12 route-файлов
...
- [ ] заполнить шапку: `title: PostgreSQL Cookbook`, `description.{ru,en}`, `basePath: /postgres-cookbook`, `repoUrl: https://github.com/dsbasko/postgres-cookbook`
- [ ] секция `brand`: `glyph: 'P'`, `level: 'Go'`, `siteUrl: https://dsbasko.github.io`, `breadcrumbRoot`, `hero{lead,accent,tail}` (ru/en), `ogImage{title, footerTag: "PostgreSQL · Go"}`
- [ ] **добавить** `brand.accent: '#336791'` + `brand.accentDark: '#5a9fd4'` (синий Postgres — единственный рычаг перекраски UI; в Kafka намеренно опущен)
- [ ] объявить все 11 модулей с `id`/`title.{ru,en}`/`description.{ru,en}` и пустыми (или плейсхолдер) `lessons` — заполнять lessons помодульно в фазе 3
- [ ] verification: `course.yaml` — валидный YAML; `id` модулей совпадут с будущими папками `lectures/`

### Task 0.3: Проверка сборки оболочки

**Files:** (только проверки, без новых файлов)

- [ ] `corepack enable && corepack prepare pnpm@9.15.0 --activate`; `pnpm install` из корня
- [ ] `make web-dev` — dev-сервер на :3000 рендерит брендинг Postgres (синий accent), пустой/скелетный TOC
- [ ] `make web-build` — статический экспорт; проверить наличие `web/out/404.html`
- [ ] `make web-typecheck` и `make web-lint` зелёные
- [ ] ⚠️ если движок/`web-check-coverage` требуют ≥1 урок (course.yaml ↔ ФС) — НЕ запускать `web-check-coverage` на этой фазе (отложить до Task 2.1, где появляется первый реальный урок); для `web-build` при необходимости создать временный плейсхолдер-урок и удалить его в фазе 2
