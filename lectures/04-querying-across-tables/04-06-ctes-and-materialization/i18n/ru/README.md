...
Это последний юнит модуля, так что собираем в одном месте грабли, на которые модуль наступал по очереди. Все они общие: запрос не падает и не выдаёт ошибку — он молча отдаёт неверное число или не ту строку. Повесь эту таблицу у себя над код-ревью.
...
> [!NOTE]
> **Проверь себя.**
> 1. Коллега переписал медленный подзапрос в `WITH` и ждёт ускорения. Что ты ему скажешь?
