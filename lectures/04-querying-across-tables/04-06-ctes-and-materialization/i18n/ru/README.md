...
Что мы упростили. Главное: разница между встраиванием и материализацией видна не в результате (он одинаков), а в **плане** — материализованный `CTE` ставит «забор», за который оптимизатор не проталкивает фильтры, и на больших данных лишний `AS MATERIALIZED` иногда мешает (сервер считает весь `CTE`, хотя снаружи нужна пара строк). Увидеть это можно только через `EXPLAIN` — это модуль 06, поэтому здесь мы лишь называем рычаги (`MATERIALIZED` / `NOT MATERIALIZED`), не меряя их. Дальше: `CTE` — про читаемость, а не про скорость; вера «перепишу подзапрос в `WITH` — станет быстрее» — миф (до PG12 материализация была всегда и иногда даже вредила). И рекурсивный `CTE` (`WITH RECURSIVE`) — отдельная большая тема: обход деревьев и графов; ему посвящён юнит 08-04.
