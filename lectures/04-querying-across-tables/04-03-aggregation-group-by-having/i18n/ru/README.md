...
Порядок логических шагов: `FROM`/`JOIN` → `WHERE` (фильтр строк) → `GROUP BY` → агрегаты → `HAVING` (фильтр групп) → `ORDER BY`.
...
Что мы упростили. Во-первых, `count(*)` и `count(колонка)` — это не «стилистика», а разные вопросы: «сколько строк» против «сколько непустых значений»; в отчётах их путают чаще всего, и баг тихий — числа выглядят правдоподобно. Во-вторых, `avg` по `numeric` мы округлили до целых центов осознанно — в проде «средний чек» с точностью до сотой копейки обычно лишний, но округлять надо явно, а не надеяться на отображение. И мы считали выручку из `orders.amount` (так в каноне), а не пересчитывали из позиций `order_items` — это другой источник и в общем случае другая сумма; в реальном отчёте важно зафиксировать, что именно считается выручкой, иначе две «правильные» цифры не сойдутся. На больших таблицах сама группировка хочет подходящих индексов и иногда упирается в память на сортировку групп — но это уже территория планов (модуль 06).
...
Дальше — юнит **04-04 «DISTINCT ON»**: достанем ровно по одной строке на группу (последний заказ каждого клиента) одним лаконичным приёмом, который есть именно в Postgres.
