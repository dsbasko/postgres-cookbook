...
Частый запрос в Brew: «покажи по последнему заказу каждого клиента». Приходит он от Евгения — тому нужна карточка клиента, и требование он теперь формулирует сам:

> **Евгений:** Мне нужна строка, а не число — max(даты) уже проходили. Не когда был последний заказ, а какой он.
...
> [!NOTE]
> Пригодится из прошлых уроков: tie-break по уникальной колонке (`id`) из 03-02 — там он давал стабильный порядок для пагинации; здесь та же идея решает, какую строку группы оставить при равных ключах сортировки. Ещё — `JOIN` из 04-01 и цены как центы-`BIGINT` из 01-01.

## Как работает DISTINCT ON

Сначала вспомним обычный `DISTINCT`. Он убирает **полные дубликаты** строк: оставляет только уникальные строки результата целиком. «Какие категории есть в меню?» — это ровно он:

```sql
SELECT DISTINCT category FROM drinks;
```

В `drinks` много напитков, но категорий мало (`coffee`, `tea`, …) — `DISTINCT` схлопывает повторы и отдаёт каждую категорию один раз. Ключевое: он смотрит на **всю** выбранную строку (тут — на одно поле `category`) и держит уникальные значения.

`DISTINCT ON` устроен иначе. `SELECT DISTINCT ON (выражение) ...` оставляет **первую** строку для каждого уникального значения `выражения` — но возвращает строку ЦЕЛИКОМ, а не только это выражение. А что считать «первой» — задаёт `ORDER BY`. Отсюда железное правило: `ORDER BY` обязан **начинаться** с того же выражения, что и `DISTINCT ON`, а дальше идёт критерий «какую строку группы выбрать».
...
> [!NOTE]
> **Проверь себя.** Убери из `LatestOrderPerCustomer` хвостовой tie-break `id DESC`, оставив `ORDER BY o.customer_id, o.created_at DESC`. На текущих данных вывод тот же? А что перестанет быть гарантированным?

> [!TIP]
> **Ответ.** На нашем сиде вывод не изменится: у Алисы два заказа с **разными** `created_at` (2025-01-15 и 2025-01-16), так что «первую» строку группы однозначно определяет уже `created_at DESC` — победит #3. Но гарантия исчезнет. Как только в группе окажутся две строки с **равным** ведущим ключом сортировки (тут — одинаковый `created_at`), `DISTINCT ON` выберет «первую» из них **произвольно** — порядок таких строк не определён, и вывод перестанет быть стабильным между прогонами. `id DESC` и есть та страховка из 03-02: он добивает порядок до полного, чтобы победитель был предсказуем всегда, а не только пока в данных нет совпадений.
