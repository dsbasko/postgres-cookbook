...
Список для акции «Мы скучаем» — клиенты, которые зарегистрировались, но так и не сделали ни одного заказа, — ушёл Евгению вчера днём. Рассылка — вчера вечером. А сегодня Евгений стоит у твоего стола, и на телефоне у него открыт профиль клиентки.

> **Евгений:** Карина Сидорова. Зарегистрировалась в январе, заказов — ноль. Идеальный адресат акции. Письма не получила. Почему её нет в списке?
>
> **Ты:** Она есть в базе. Запрос простой: клиенты, заказы, джойн по customer_id…
>
> **Евгений:** Мне не нужен джойн, мне нужна Карина. Акция затевалась ради таких, как она. Сколько ещё человек список потерял?

На последний вопрос ответа нет — и это хуже всего. Дмитрий подходит с кружкой и молча кивает на экран.

> **Дмитрий:** Покажи запрос.

Ты показываешь. `customers JOIN orders ON …` — всё как в учебнике.

> **Дмитрий:** Запрос честный. INNER JOIN возвращает только пары «клиент — заказ». У Карины заказов нет, пары нет — и строки нет. Он её не потерял. Он её выбросил. По правилам.
>
> **Ты:** А как спросить «все клиенты — даже те, у кого пары нет»?
>
> **Дмитрий:** Одним словом. Его и разберём — а заодно все четыре вида JOIN: кто кого сохраняет и кто кого выбрасывает. Евгений, к обеду будет полный список.
>
> **Евгений:** К обеду. И сразу цифру, сколько их всего, — Эмиль спросит про охват.
...
> - **либо** оставь условие в `WHERE`, но явно пропусти беспарные: `WHERE o.status = 'paid' OR o.id IS NULL`. Это не эквивалент варианта с `ON`: клиент, у которого заказы есть, но ни один не `'paid'` (наш Борис с его `created`), здесь выпадет — его `o.id` не `NULL`. Нужен и он строкой с `NULL` — бери вариант с `ON`.
>
