...
Обед. Ты катишь релиз: две миграции, обе после ревью, на вид безобидны — `ADD COLUMN ... DEFAULT 'active'` и одна строка `ALTER TABLE orders ALTER COLUMN amount TYPE numeric(12,2)`. Релиз уходит в 12:33.

> **Руслан (в чате, 12:41):** Кассы. Обе. 12:41.
>
> **Ты:** Смотрю. Из нового — только релиз. Но там смена типа колонки, она же безобидная.
>
> **Павел:** ALTER TYPE на orders. ACCESS EXCLUSIVE, переписывание всей таблицы. Кассы стоят за твоей миграцией. Убиваю.

Восемь минут — от релиза до kill — обе кассы не пробили ни одного заказа: таблица в десятки миллионов строк переписывалась под блокировкой. Вечером — твой первый постмортем, в графе «автор» — ты. Павел молча достаёт новый бумажный блокнот и записывает дату.

> **Ботыр:** Помнишь, в первый день: «у нас так касса вставала»? Это было два раза. Видимо, придётся пересчитать.
>
> **Дмитрий:** Счёт потом. Правило постмортема: виноват способ, не человек. Строка прошла ревью. Способ — переписывание горячей таблицы в обед.
>
> **Ты:** В файле миграции обе строки выглядят одинаково. Как отличать?
>
> **Дмитрий:** Не по синтаксису. Одни ALTER правят метаданные — это мгновенно. Другие переписывают таблицу под блокировкой. Учимся различать заранее.
...
| `ALTER COLUMN ... SET NOT NULL` (существующая колонка) | сканирует таблицу, проверяя, что NULL нет | блокировка на время скана (PG12+ пропустит при валидном `CHECK (col IS NOT NULL)`) |
...
- **Нюансы версий.** `ADD COLUMN` с **волатильным** дефолтом (`now()`, функция) уже переписывает таблицу. А `NOT NULL` зависит от того, как он появился: `ADD COLUMN ... NOT NULL` с константным дефолтом мгновенен (новая колонка и так без `NULL` — это и показывает наше демо), а `ALTER COLUMN ... SET NOT NULL` на существующей колонке сканирует таблицу, проверяя старые строки (PG12+ пропустит скан, если есть валидный `CHECK (col IS NOT NULL)`).
...
На этом модуль **02 «Схема, DDL и ограничения»** закрыт: `IDENTITY`/`DEFAULT`, `NOT NULL`/`PK` и выбор ключа, внешние ключи, `UNIQUE`/`CHECK`, генерируемые столбцы/домены и мышление миграций. Постмортем твоего инцидента дочитывает Ботыр — и задерживается у стола.

> **Ботыр:** Не расстраивайся: самый дорогой наш инцидент — вообще одна строка. UPDATE без WHERE. Знаю. Я автор.

Чем эта строка обошлась Brew и какая дисциплина спасает от таких строк — с этого начнётся модуль **03 «CRUD-беглость»**: уверенные `INSERT ... RETURNING`, `SELECT` с пагинацией, безопасные `UPDATE`/`DELETE`, `upsert` и трезвая семантика `NULL`.
