...
> **Анна (в чате, 12:41):** Кассы. Обе. 12:41.
>
> **Ты:** Смотрю. Из нового — только релиз. Но там смена типа колонки, она же безобидная.
>
> **Зоя:** ALTER TYPE на orders. ACCESS EXCLUSIVE, переписывание всей таблицы. Кассы стоят за твоей миграцией. Убиваю.

Восемь минут — от релиза до kill — обе кассы не пробили ни одного заказа: таблица в десятки миллионов строк переписывалась под блокировкой. Вечером — твой первый постмортем, в графе «автор» — ты. Зоя молча достаёт новый бумажный блокнот и записывает дату.

> **Даня:** Помнишь, в первый день: «у нас так касса вставала»? Это было два раза. Видимо, придётся пересчитать.
>
> **Марат:** Счёт потом. Правило постмортема: виноват способ, не человек. Строка прошла ревью. Способ — переписывание горячей таблицы в обед.
>
> **Ты:** В файле миграции обе строки выглядят одинаково. Как отличать?
>
> **Марат:** Не по синтаксису. Одни ALTER правят метаданные — это мгновенно. Другие переписывают таблицу под блокировкой. Учимся различать заранее.
...
На этом модуль **02 «Схема, DDL и ограничения»** закрыт: `IDENTITY`/`DEFAULT`, `NOT NULL`/`PK` и выбор ключа, внешние ключи, `UNIQUE`/`CHECK`, генерируемые столбцы/домены и мышление миграций. Постмортем твоего инцидента дочитывает Даня — и задерживается у стола.

> **Даня:** Не расстраивайся: самый дорогой наш инцидент — вообще одна строка. UPDATE без WHERE. Знаю. Я автор.
