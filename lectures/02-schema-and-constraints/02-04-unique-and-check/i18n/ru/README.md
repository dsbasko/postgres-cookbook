...
Ночной импорт прайса отработал «успешно» — так, по крайней мере, сказал скрипт. Заливал его Паша, закупщик Brew: поставщики, склад, ночные выгрузки.

> **Паша (в чате, 23:50):** Прайс загрузил, всё зелёное. Файл как файл.
>
> **Анна (в чате, 08:12):** Пробиваю раф из нового прайса — ноль рублей. Так и надо?

Анна — управляющая первой кофейни, цены своего меню она знает до рубля. И раф — не единственная жертва: у пары напитков цена `0` (касса всё утро отдавала их бесплатно), в меню — дубли одной позиции, у одного напитка — размер `XXL`, которого нет ни в одном стакане. Всё это доехало до кассы, потому что база **разрешила**: схема ничего не запрещала. Лечится это не кодом импортёра, а двумя декларативными ограничениями прямо в таблице: `UNIQUE` против дублей и `CHECK` против бессмысленных значений.
