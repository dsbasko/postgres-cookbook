...
Что мы спрятали под NOT NULL: `CHECK` **пропускает `NULL`**. Условие нарушено, только если оно вычислилось в `false`; `NULL` даёт `unknown`, а `unknown` ≠ `false` — значит, строка с `price = NULL` прошла бы `CHECK (price > 0)` насквозь. Именно поэтому в `check_drink` колонки помечены `NOT NULL`: без этого `CHECK` ловит «`-5`», но молча пропускает «ничего». В проде эту пару — `NOT NULL` рядом с `CHECK` — держат вместе осознанно. Ещё два момента, которыми занимается твой DBA: добавить `CHECK`/`UNIQUE` на большую живую таблицу — это сканирование с блокировкой (поэтому констрейнт сначала вешают `NOT VALID`, а потом валидируют отдельно — см. 02-06); а сложную бизнес-валидацию («скидка не больше суммы заказа») не всегда стоит загонять в `CHECK` — её сложнее эволюционировать, чем код, и она не видит другие таблицы (для «между строками» нужен EXCLUDE-констрейнт или триггер). Правило: `UNIQUE`/`CHECK`/`NOT NULL` — для инвариантов уровня одной строки и колонки; они дёшевы, декларативны и неотключаемы — пользуйся ими по умолчанию.
