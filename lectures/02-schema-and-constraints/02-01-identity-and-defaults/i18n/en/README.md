...
Two dedicated tables (DDL in `schema.sql`) — the Brew canon is byte-compatible with kafka-cookbook and its ids must not be touched, so we show the contrast on new ones:
...
What we simplified: we showed that `BY DEFAULT` silently desyncs the counter, but we didn't drive it to an actual collision (`duplicate key`) — that's tens of thousands of inserts. In production your DBA or migration script handles it: after any data load with explicit ids you bump the sequence via `SELECT setval(pg_get_serial_sequence('t','id'), (SELECT max(id) FROM t))` — exactly what our `schema/seed.sql` does for the canon `orders` table. The practical takeaway for a developer: start new tables on `GENERATED ALWAYS AS IDENTITY` — it forbids the app from touching the key and kills a whole class of bugs. Reach for `BY DEFAULT` deliberately and only where you need to temporarily substitute ids (import, replicating test data), and don't forget the `setval` afterward. `OVERRIDING SYSTEM VALUE` exists, but it's an emergency hatch, not a daily tool.
