...
- Desync is fixed with `setval`: after any data load with explicit ids you bump the sequence via `SELECT setval(pg_get_serial_sequence('t','id'), (SELECT max(id) FROM t))` — exactly what our `schema/seed.sql` does for the canon `orders` table.
