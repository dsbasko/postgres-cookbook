...
Brew переезжал со старой таблицы заказов и переливал историю скриптом. Таблица была на `serial`, скрипт вставлял строки с **явными** id из выгрузки (`1001..1500`), а вот последовательность за этими значениями никто не подвинул — она осталась на старом месте. Несколько недель всё работало, заказы шли. А потом одно утро началось с рапорта в командном чате:

> **Павел (в чате):** orders_pkey. duplicate key. с шести утра сыплется.

Последовательность доросла до `1500`, попробовала выдать уже занятый id — и упёрлась в собственную историю. Классический «серийный» прострел в ногу.
