...
- Рассинхрон лечит `setval`: после любой загрузки данных с явными id последовательность подвигают через `SELECT setval(pg_get_serial_sequence('t','id'), (SELECT max(id) FROM t))` — ровно это делает наш `schema/seed.sql` для канон-таблицы `orders`.
