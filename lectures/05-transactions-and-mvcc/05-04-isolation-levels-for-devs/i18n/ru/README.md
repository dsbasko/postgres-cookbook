# 05-04 — Уровни изоляции на практике
...
> **Руслан (в чате, 12:30):** Зал. Пустой. Очередь до двери. Оба бариста на складе.

Руслан управляет этой кофейней: для него сломанный инвариант — не строка в логе, а очередь гостей у пустой стойки. Смену он не винит, и правильно: бариста рассуждали безупречно — подвёл способ, которым две одновременные транзакции читают общий зал.
...
Прежде чем разобрать, чем именно опасен write-skew, — короткий диалог из-за соседнего стола.

> **Ботыр:** А давайте «просто» всё на SERIALIZABLE — и забудем про аномалии?
>
> **Дмитрий:** Можно. Но больно. Цена — ретраи. Завтра посчитаем.

«Завтра посчитаем» — это про следующий юнит: у строгости `SERIALIZABLE` есть цена, и видно её только под нагрузкой.
