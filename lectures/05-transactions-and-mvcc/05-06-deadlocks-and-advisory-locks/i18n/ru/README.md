...
Postgres сам выбрал жертву (здесь — A) и откатил её транзакцию; B при этом прошла и закоммитила. (Номера процессов/транзакций в `DETAIL` будут свои, и жертвой может оказаться B — выбор за планировщиком.) После демо восстанови канон: `make ... T=db-reset`.

## Заборчик

Порядок шагов в сессиях держат `\prompt` — в реальной гонке всё происходит за миллисекунды, и какую транзакцию назначить жертвой, решает Postgres (как правило — ту, что откатить «дешевле»). Что мы упростили: дедлок здесь — на блокировках строк, но он возможен и на любых блокировках (внешние ключи без индекса, эскалация на уровне `ALTER TABLE`, те же advisory-локи, взятые крест-накрест). Бороться лучше **профилактикой** (единый порядок захвата, короткие транзакции, индексы под FK — см. 06), а ретрай на `40P01` держать как страховку, а не как стратегию. И помни про разницу: `40001` (05-04/05-05) — это про *логические* конфликты сериализации, его не предотвратить порядком блокировок; `40P01` — про *физический* цикл ожидания, и вот он порядком как раз лечится. Advisory-локи тоже не бесплатны: session-level лок легко «утечёт» (забыли unlock, или коннект вернулся в пул с висящим локом) — поэтому в приложениях почти всегда берут `pg_advisory_xact_lock`, который снимается сам.
