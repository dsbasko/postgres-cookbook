...
Утро, разбор у доски. Павел уже здесь — раньше всех, с распечаткой ночного серверного лога и раскрытым блокнотом.

> **Павел:** Ночь. Возвраты. Половина откатилась. В логе — взаимная блокировка.
>
> **Ботыр:** Мой воркер берёт сначала заказ, потом товар. На тесте летало.
>
> **Ты:** А мой — сначала товар, потом заказ. Тоже по учебнику.
>
> **Дмитрий:** Покажите оба.

Два воркера, две аккуратные транзакции — и по отдельности ни одной ошибки.

> **Дмитрий:** Порознь оба безупречны. Но ночью они взяли одни строки во встречном порядке — и заперли друг друга. Виноват порядок, не человек.
>
> **Ты:** И как чинить — кому-то переписывать воркер?
>
> **Дмитрий:** Обоим, одинаково: блокировать всегда в одном порядке. А на пересчёт остатков — общие ворота, чтобы воркеры вставали в очередь, а не сталкивались.

Павел записывает инцидент в блокнот. За неделю страниц в нём прибавилось.

То, что случилось ночью, называется **дедлок** (deadlock): две транзакции держат ресурсы друг друга и ждут навстречу — не продолжит никто.
...
Это последний юнит модуля 05. Павел закрывает блокнот.

> **Павел:** Конкуренцию пережили. Теперь — почему один запрос летает, а соседний ползёт.

Дальше — модуль **06 «Индексы и EXPLAIN»**: блокировки и снимки разобраны, пора понять, как Postgres *находит* строки, и научиться читать план запроса (`EXPLAIN ANALYZE` с буферами — в PG18 по умолчанию).
