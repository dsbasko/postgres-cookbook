...
Что мы упростили: пояса здесь выбраны с фиксированным зимним смещением (Москва `+03` круглый год с 2014-го, Нью-Йорк зимой `-05`), чтобы вывод воспроизводился дословно. В реальности есть переход на летнее время: в Нью-Йорке летом было бы `-04`, и тот же UTC-инстант показался бы на час иначе — Postgres это учитывает по имени зоны (`America/New_York`), но именно поэтому **храни имя зоны, а не числовое смещение**, если тебе важна локальная дата для будущих событий. И ещё: в проде форматирование времени для пользователя живёт в слое представления (его пояс берётся из профиля/`Accept-Language`/настроек), а БД и бэкенд оперируют инстантами в UTC. Здесь мы крутим `SET TIME ZONE` руками только чтобы увидеть механику — в приложении ты так почти никогда не делаешь.
