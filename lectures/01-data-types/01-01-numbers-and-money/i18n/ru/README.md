...
`float8` (он же `double precision`) хранит числа в двоичной плавающей точке. Десятичные дроби вроде `0.1` в двоичке периодические — их приходится округлять, и при сложении ошибки округления всплывают наружу. Классическая демонстрация: `0.1 + 0.2` даёт `0.30000000000000004`, а не `0.3`. Сравнение `0.1 + 0.2 = 0.3` возвращает `false`. На одном чеке это семнадцатый знак после запятой; на тысячах заказов — та самая копеечная дыра с распечаток Виктора.
