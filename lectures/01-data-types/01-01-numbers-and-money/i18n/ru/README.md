...
Первый отчёт по выручке ушёл утром Эмилю — основателю Brew и в прошлом бариста. До обеда он поднимается в опенспейс сам. В руках — две распечатки.

> **Эмиль:** Два отчёта — два числа. Касса говорит одно, ваши цифры — на пару копеек меньше, и так в каждой строке. Где мои деньги?
>
> **Ты:** Может, касса округляет?
>
> **Эмиль:** Касса пробивает то, что заплатил гость. Гости платят точно.

Дмитрий пробегает глазами колонку итогов, берёт салфетку и пишет одну строку: `0.1 + 0.2 ≠ 0.3`.

> **Ты:** В каждом сложении?
>
> **Дмитрий:** В каждом. Никто не крал. Складывали во float.
>
> **Эмиль:** Мне всё равно, как это называется. Сделайте, чтобы копейки сходились.

Эмиль уходит вниз, к гостям; распечатки остаются у тебя.
...
> **Павел — в ревью, одной строкой:** Центы — правильно. Валюту и округление в проде спросят с тебя.
...
Копейки сходятся — распечатки Эмиля можно возвращать. Дальше — юнит **01-02 «text, boolean и тизер NULL»**: разберём три «скучных» типа, на которых на самом деле спотыкаются приложения, — почему мы держим `text`, а не `char(n)`, что такое трёхзначная логика `boolean` и почему `NULL` — это не «пусто», а «неизвестно».
