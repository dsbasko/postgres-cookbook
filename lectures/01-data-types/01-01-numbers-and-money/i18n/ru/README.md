...
Первый отчёт по выручке ушёл утром Виктору — основателю Brew и в прошлом бариста. До обеда он поднимается в опенспейс сам. В руках — две распечатки.

> **Виктор:** Два отчёта — два числа. Касса говорит одно, ваши цифры — на пару копеек меньше, и так в каждой строке. Где мои деньги?
>
> **Ты:** Может, касса округляет?
>
> **Виктор:** Касса пробивает то, что заплатил гость. Гости платят точно.

Марат пробегает глазами колонку итогов, берёт салфетку и пишет одну строку: `0.1 + 0.2 ≠ 0.3`.

> **Ты:** В каждом сложении?
>
> **Марат:** В каждом. Никто не крал. Складывали во float.
>
> **Виктор:** Мне всё равно, как это называется. Сделайте, чтобы копейки сходились.

Виктор уходит вниз, к гостям; распечатки остаются у тебя.

Скрипт выгрузки писали задолго до тебя: суммы в нём складывает `float`. Цель юнита — закрыть этот класс багов: понять, почему `float` нельзя для денег, и выбрать представление, которое точно считается и удобно ложится в Go. В Postgres есть `numeric` (точный), но приложения держат деньги проще — целым числом центов: так устроена схема Brew, `drinks.base_price` — `BIGINT` в центах.

## Почему float ломает деньги

`float8` (он же `double precision`) хранит числа в двоичной плавающей точке. Десятичные дроби вроде `0.1` в двоичке периодические — их приходится округлять, и при сложении ошибки округления всплывают наружу. Классическая демонстрация: `0.1 + 0.2` даёт `0.30000000000000004`, а не `0.3`. Сравнение `0.1 + 0.2 = 0.3` возвращает `false`. На одном чеке это семнадцатый знак после запятой; на тысячах заказов за месяц — та самая копеечная дыра с распечаток Виктора.
...
`float` дал `0.30000000000000004` и `= 0.3` → `false` — ту самую копеечную дыру с распечаток: не кража, а семнадцатый знак после запятой. `numeric` точен. А деньги Brew живут целыми центами: меню разворачивается в `₽.коп` без потерь, итог заказа #1 — `970` центов = `9.70`.

## Заборчик

> **Зоя — в ревью, одной строкой:** Центы — правильно. Валюту и округление в проде спросят с тебя.
...
Копейки сходятся — распечатки Виктора можно возвращать. Дальше — юнит **01-02 «text, boolean и тизер NULL»**: разберём три «скучных» типа, на которых на самом деле спотыкаются приложения, — почему мы держим `text`, а не `char(n)`, что такое трёхзначная логика `boolean` и почему `NULL` — это не «пусто», а «неизвестно».
