...
`numeric` — не «плохой» тип: для денег он точен, и хранить суммы в `numeric(12,2)` — совершенно нормально. Мы выбираем целые центы по двум причинам: они ложатся в Go `int64` без обёртки `pgtype.Numeric`, и арифметика над ними быстрее. Что мы упростили: в реальном платёжном коде есть валюты (центы доллара ≠ копейки рубля — нужен код валюты рядом с суммой), правила округления (банковское округление половин), дробные цены за единицу веса и налоги, где промежуточные вычисления всё же ведут в `numeric`, а в центы сворачивают на финальном шаге. В проде твой расчётный модуль хранит и валюту, и масштаб; здесь мы держим одну валюту и целые центы, чтобы не уводить урок в сторону. Незыблемо одно: **деньги никогда не считаются во `float`**.
