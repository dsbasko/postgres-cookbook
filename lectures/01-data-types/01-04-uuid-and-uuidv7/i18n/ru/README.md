...
Цель юнита — выбрать ключ осознанно. Числовой `IDENTITY` (его мы видели на канон-таблицах) хорош, но раскрывает порядок и количество. Случайный `gen_random_uuid()` (v4) непредсказуем, но не сортируется и фрагментирует индекс. PG18 добавил `uuidv7()` — `uuid`, в начало которого зашит timestamp: он так же непредсказуем в хвосте, но **монотонно растёт во времени**, поэтому годится как сортируемый по времени первичный ключ. Демонстрируем его на **новой** таблице — канон байт-совместим с kafka-cookbook (`customers.id` там `BIGINT`), и трогать его нельзя.
...
Кстати, `schema.sql` этого юнита — первый в курсе, что добавляет **свою** таблицу: `make db-reset` накатывает её через `brew.Apply(ctx, pool, ddl)` (канон → DDL юнита → seed), а сам DDL `main.go` читает рядом с собой через `runtime.Caller`.
...
Что мы упростили: монотонность `uuidv7` — это про **порядок**, а не про безопасность. `uuidv7` не прячет время создания (его легко достать), поэтому если важно не раскрывать «когда» — это не тот инструмент. И наоборот, v4 раскрывает меньше, но проигрывает в локальности индекса. В проде выбор ключа — это компромисс: `IDENTITY` (компактный, быстрый, но раскрывает порядок/объём и плохо мерджится между базами), `uuidv7` (распределённо-дружелюбный, сортируемый, но крупнее и светит время), `uuid` v4 (максимально непредсказуемый, но фрагментирует индекс). Для распределённых вставок и публичных идентификаторов `uuidv7` — хороший дефолт; для узких внутренних таблиц `IDENTITY` часто всё ещё лучше. Канон Brew намеренно остаётся на `BIGINT` ради байт-совместимости с kafka-cookbook — новые идеи мы пробуем на новых таблицах.
...
- Современные идиомы (`uuidv7`) демонстрируй на новых таблицах; канон Brew (`*.id BIGINT`) не трогай — это держит handoff с kafka-cookbook.
