...
Отчёт «сколько у каждого клиента заказов» в Brew написали быстро — и по нему уже ушла рассылка лояльности. Утром в командном чате появился Евгений.

> **Евгений (в чате):** Сверяю рассылку лояльности с отчётом «заказы по клиентам». Карина Сидорова — не получила. Профиль у меня открыт: зарегистрирована, а в отчёте её нет. Куда она делась?
>
> **Ты:** В `customers` она есть, строка на месте. Куда она пропадает по дороге в отчёт?
>
> **Дмитрий:** А чем посчитаны заказы — `count(*)` или `count` по колонке? Пересчитай через `count(*)` и сравни. И посмотри, что у Карины в `order_id`.

Пересчитываешь: `count(*)` даёт на строку больше, чем прежний `count(order_id)`. Лишняя строка — Карина: клиент на месте, заказов нет, а её пустой `order_id` прежний счёт молча пропускал.

> **Евгений (в чате):** Вернулась, вижу. Запишу себе: в базе живёт какой-то NULL и ест моих клиентов. Разберётесь — объясните по-человечески.
