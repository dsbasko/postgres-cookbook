...
Это только тизер. Полную семантику `NULL` — `NOT IN` с `NULL` (классическая дыра, возвращающая пустоту), `COALESCE`/`NULLIF`/`IS DISTINCT FROM`, трёхзначную логику в `WHERE`/`CHECK`/уникальных индексах — разберём в 03-06. Здесь важно усвоить одно правило: **проверка на отсутствие значения — только `IS NULL` / `IS NOT NULL`, никогда `= NULL`**. Что до `char(n)` — в проде ты его почти никогда не встретишь по делу; если видишь `char(n)` в чужой схеме, это чаще всего историческая ошибка, а не намеренный выбор.
