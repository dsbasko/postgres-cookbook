...
Редкая неделя: в Brew ничего не горит. Евгений спускается в опенспейс без пожара — с планом и тремя запросами сразу.

> **Евгений:** Смотри, что накопилось. Напиткам нужны размеры — S, M, L, это каждый второй заказ. Блогу — теги. И гости просят сироп, корицу, «без крышки» — куда это класть?

Три разные задачи — и три разных «контейнерных» типа в Postgres: `enum` для фиксированного набора размеров, массив для тегов, `jsonb` для гибких опций вроде «молоко овсяное, +1 шот». Каждый удобен ровно в своей нише, и каждый легко применить не туда.
...
А в командном чате тем временем всплывает сообщение.

> **Евгений (в чате):** Вторая точка согласована — Эмиль подписал план. На следующей неделе принесу список задач: меню, клиенты, заказы — всё должно уметь работать на два адреса.
