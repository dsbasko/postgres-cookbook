...
Руслан пишет из BREW-CENTRAL в разгар дня — касса не берёт латте.

> **Руслан (в чате, 13:20):** Латте на витрине — минус пять рублей. Пробивать?
>
> **Ты:** Минус пять? Откуда у латте отрицательная цена?
>
> **Ботыр:** Никто её не ставил. Карточку я на прошлом спринте перевёз в `doc jsonb` — так гибче, схему не трогаешь. Цена теперь ключ внутри документа.
>
> **Дмитрий:** А когда она стала отрицательной?
>
> **Ботыр:** Вот этого никто и не скажет. В колонке стоял `CHECK (price > 0)`. Внутри документа его нет — аналитик сети пробовал навесить, на `jsonb` не ложится.
>
> **Павел:** И это ещё полбеды. Меняешь один флаг в карточке — переписываешь весь документ. На горячей таблице счёт по WAL я тебе принесу.
>
> **Ботыр:** На тесте летало. Десять карточек — мгновенно.
>
> **Дмитрий:** Десять. В проде их тысячи, и каждую правят десять раз в день. Считать будем прод, не тест.
>
> **Ботыр:** Знаю. Разбор напишу сам. И правило туда: поле, по которому есть инвариант, — это колонка, а не ключ в `jsonb`. Не верь слову «просто» — я же сам этому учил.
>
> **Павел:** Запиши. И поля с ограничением верни в схему. Попрошу отдельно.

У гибкости `jsonb` есть цена — и она из двух частей.
