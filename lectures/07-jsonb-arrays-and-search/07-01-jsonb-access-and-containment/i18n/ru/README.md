...
`jsonb` соблазняет сложить в одну колонку всё подряд — и тут же мстит. Поле, по которому ты регулярно фильтруешь, считаешь или джойнишь, почти всегда должно быть **обычной колонкой**, а не ключом внутри `jsonb`: колонку проверит `CHECK` и `NOT NULL`, на ней живёт обычный B-tree, и план запроса предсказуем. Внутри `jsonb` ничего этого нет даром: containment-поиск без GIN — это `Seq Scan` по всей таблице, а `?`-фильтр по отсутствующему ключу легко даёт сюрпризы из трёхзначной логики (см. 03-06). Правило простое: **`jsonb` — для по-настоящему бесформенного и разреженного; всё, на что опирается логика приложения, выноси в колонки**. В проде размер документа тоже не бесплатен — об этом ровно следующий юнит.
