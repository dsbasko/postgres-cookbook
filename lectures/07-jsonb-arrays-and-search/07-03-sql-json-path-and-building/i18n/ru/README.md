...
jsonpath мощный — и тем соблазняет таскать в `jsonb` то, что давно просится в колонки. Если ты регулярно фильтруешь по `$.kcal > 100` или `? (@.name == "milk")`, это сигнал: данные структурированы, и им место в нормальных столбцах с обычным B-tree (07-02). Для самого `jsonb` помни про индекс: `@?`/`@@`/`@>` на большой таблице без GIN — это `Seq Scan` (06-05). Сборка (`jsonb_agg`) прямо в SQL удобна для API-ответа, но не увлекайся: тяжёлую агрегацию всего каталога в один документ лучше кэшировать, а не пересобирать на каждый запрос — в проде это нагрузка на CPU базы. И `JSON_TABLE` — это PG17: если читаешь чужой код с ним, не приписывай его «новинкам 18».
