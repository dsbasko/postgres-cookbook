...
В Brew появились рецепты напитков: у каждого — массив ингредиентов с граммовками, вложенный в `jsonb`. С утра Руслан пересылает в чат запрос от тренера бариста:

> **Руслан (в чате, 08:40):** Тренер спрашивает: в каких рецептах молоко и что тяжелее ста граммов. Новичкам — аллергены. К открытию.

Вопросы простые, когда меню в руках, но для базы каждый из них — «пройди по вложенному массиву ингредиентов и ответь». Цепочкой `->`/`#>` из 07-01 это пишется неуклюже: достать массив, развернуть, отфильтровать, собрать обратно. Для вложенных документов в Postgres есть язык получше — **jsonpath**.
...
> **Ты:** `jsonb_set` правит документ прямо в базе?
>
> **Дмитрий:** Возвращает новый. Чем оборачивается перезапись документа целиком — видели на прошлой неделе.
