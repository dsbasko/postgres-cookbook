...
Массив соблазняет компактностью — и мстит, когда тег перестаёт быть просто ярлыком. Как только тегу нужны свои атрибуты (когда повешен, кем, с каким весом), нужен справочник с проверкой опечаток (FK), или ты регулярно считаешь/джойнишь по тегам — это сигнал к junction-таблице. Массив же не даст ни внешнего ключа на элемент, ни уникальности внутри (в `{coffee, coffee}` никто не запретит дубль), а аналитика по нему всегда идёт через `unnest`. Обратная крайность тоже бывает вредной: junction на простые неизменные ярлыки с единственным вопросом «есть ли тег» — это лишний join на ровном месте. Практичное правило: **junction по умолчанию для всего, что имеет связи и атрибуты; массив — для коротких простых списков, где единственная операция — `@>`/`= ANY`**. В проде «массив, по которому мы теперь джойним и считаем» обычно мигрируют в junction — и DBA попросит сделать это раньше, чем он распухнет.
