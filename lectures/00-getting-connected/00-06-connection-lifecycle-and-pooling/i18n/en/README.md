...
> **Botyr:** The site won't take orders. Now the register catches the same FATAL — and it did nothing wrong.
>
> **Pavel:** I see it. Backends hit the limit. All of them — yours.
>
> **Dmitry:** We look from both sides. Pavel — the server. You — the pool.
>
> **You:** Our pool is empty. The site goes around it: the launch code calls pgx.Connect on every HTTP request.
>
> **Pavel:** So every guest's click is a new process on my server. Ceiling. Refusal — for everyone.
>
> **Dmitry:** That code is older than our pool. First understand, then fix.
...
>
> **Pavel:** A hundred processes. Memory. The scheduler.
>
> **Dmitry:** Hence the pool.
...
Late in the evening, once the site is taking orders again, a message pops up in the team chat from Emil — the founder you know so far only from the framed receipt of order #1.

> **Emil (in chat):** The site is alive, guests are ordering. In the morning I want our first report: how much we made today. Down to the kopek.

That closes module **00 "Getting connected"**: you have a sandbox, psql at hand, a working pipeline — "SQL by hand → sqlc → typed pgx code" — and an understanding of what the pool does with connections. Next up is module **01 "Data types"**: which type to pick and why, starting with Emil's report — with money, where `numeric` vs `float` decides whether Brew's register balances down to the kopek.
