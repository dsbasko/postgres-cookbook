...
> **Даня:** Сайт не принимает заказы. Теперь и касса ловит тот же FATAL — а она ни при чём.
>
> **Зоя:** Вижу. Бэкенды упёрлись в лимит. Все — ваши.
>
> **Марат:** Смотрим с двух сторон. Зоя — сервер. Ты — пул.
>
> **Ты:** Наш пул пустой. Сайт ходит мимо него: в коде запуска — pgx.Connect на каждый HTTP-запрос.
>
> **Зоя:** Каждый клик гостя — новый процесс на моём сервере. Потолок. Отказ — всем подряд.
>
> **Марат:** Код старше нашего пула. Сначала — разобраться, потом — чинить.
...
>
> **Зоя:** Сто процессов. Память. Планировщик.
>
> **Марат:** Поэтому пул.
...
Поздно вечером, когда сайт снова принимает заказы, в командном чате всплывает сообщение от Виктора — основателя, знакомого тебе пока только по чеку заказа #1 в рамке.

> **Виктор (в чате):** Сайт ожил, гости заказывают. Утром хочу первый отчёт: сколько мы заработали за день. До копейки.

На этом модуль **00 «Подключение и ориентация»** закрыт: есть песочница, psql под рукой, рабочий конвейер «SQL руками → sqlc → типизированный pgx-код» и понимание, что пул делает с соединениями. Дальше — модуль **01 «Типы данных»**: какой тип выбрать и почему, начиная с отчёта для Виктора — с денег, где `numeric` против `float` решает, сойдётся ли у Brew касса до копейки.
