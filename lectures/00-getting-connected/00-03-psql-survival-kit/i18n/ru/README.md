...
Утро после настройки песочницы — и первый боевой тикет. Даня, не поворачиваясь от монитора, перекидывает его тебе.

> **Даня:** Держи. «На сайте пропал колд брю из меню». Поздравляю — боевое крещение у нас выглядит так.
>
> **Ты:** Смотреть код витрины?

Ты уже тянешься к репозиторию, но Марат, проходя мимо с кружкой, кивает на терминал.

> **Марат:** Код подождёт. Что база ответила? Три команды — и узнаешь.

Марат прав: прежде чем лезть в код, стоит на тридцать секунд заглянуть в саму базу — есть ли там вообще этот напиток, какой у него `stock`, не сломалась ли структура таблицы. Можно открыть Adminer и кликать мышкой, но это медленно и не ложится в терминальный workflow. Рабочий инструмент для такого — `psql`: официальный консольный клиент Postgres, который ставится вместе с сервером (`brew install libpq` на macOS).
...
> [!NOTE]
> Опирается на 00-02: песочница поднята (`docker compose up -d`), и граница
> «клиент шлёт SQL — сервер исполняет» уже знакома. psql — ещё один клиент к тому
> же серверу, только интерактивный, для работы руками.
...
> [!NOTE]
> **Проверь себя.** Что из этого psql отправит на сервер, а что исполнит сам, не
> отправляя: `\dt`, `SELECT count(*) FROM drinks;`, `\d drinks`, `\timing`? И
> почему `\d drinks` нельзя вызвать из `pgx` в Go?

> [!TIP]
> **Ответ.** Как текст на сервер уходит только `SELECT count(*) FROM drinks;` —
> это SQL. Остальные три начинаются с `\` — это мета-команды, их разбирает сам
> psql. Но «сам» не значит «без сервера»: ради `\dt` и `\d drinks` psql шлёт на
> сервер собственные запросы к каталогу (запусти `psql -E` — увидишь их);
> целиком на клиенте исполняется только `\timing`. Из `pgx` мета-команды не
> вызвать ровно поэтому: `\d` — не SQL, сервер такой команды не понимает; из
> кода те же факты добывают запросом к `pg_catalog`/`information_schema`.
