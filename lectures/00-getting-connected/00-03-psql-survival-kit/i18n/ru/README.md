...
> [!TIP]
> **Ответ.** На сервер уходит только `SELECT count(*) FROM drinks;` — это SQL.
> Остальные три начинаются с `\` — это мета-команды, их исполняет сам psql на
> клиенте. Из `pgx` их не вызвать ровно поэтому: `\d` — не SQL, сервер такой
> команды не понимает; то же самое из кода добывают запросом к
