...
Финал курса — конкретный. Ты сам откроешь `PUBLICATION` и передашь поток изменений Brew в соседний курс [`kafka-cookbook`](https://github.com/dsbasko/kafka-cookbook): Postgres отдаёт эстафету, Kafka принимает. Две кофейные истории — один мир, одна модель данных. Поэтому базовые таблицы Brew в этом курсе байт-в-байт совпадают со схемой соседнего: переименуешь колонку — и эстафета сломается. Об этом будет отдельный разговор в капстоне, а пока просто запомни, что у курса есть выход наружу, а не только внутренняя кухня.
