...
Brew начинался как одна кофейня, а теперь становится сетью: московская точка (`BREW-CENTRAL`) работает, вторая — питерская (`BREW-NORTH`) — уже заведена в базе и ждёт открытия, склад под неё заполняется. Плюс общее меню, общая база клиентов и собственное приложение — касса, сайт, промо-рассылки, отчёты. Всё это ходит запросами в ту самую «одну базу» с экскурсии. Дальше Brew будет расти — и это не просто фон. Чем больше Brew, тем дороже ошибка: то, что на одной кассе было незаметно, на сети оборачивается потерянными заказами и кассой, которая встала в обед. Поэтому боль в курсе нарастает от модуля к модулю вместе с самим Brew.
