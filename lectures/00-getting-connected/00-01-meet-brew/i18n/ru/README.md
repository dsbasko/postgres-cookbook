...
![Постер кофейни Brew](/posters/00-00.png)
...
> **Дмитрий:** Ноутбук выдали, пропуск сработал — для десяти утра уже неплохо. Я Дмитрий, тимлид бэкенда. Экскурсия короткая: вся наша система — вот.
>
> **Ты:** Одна база?
>
> **Дмитрий:** Одна база, девять таблиц: заказы, позиции, меню, клиенты, склад. Люди почему-то ждут, что за сетью кофеен стоит что-то грандиозное.
>
> **Ботыр:** И outbox. Про outbox он всегда забывает, а это лучшая наша таблица — поймёшь ближе к зиме. Я Ботыр, второй бэкендер, теперь нас трое. Совет на первую неделю: не верь здесь слову «просто». «Просто добавь колонку», «просто поправь цену» — у нас так касса вставала.
>
> **Дмитрий:** Это было один раз.
>
> **Ботыр:** Это было два раза.

Из-за перегородки, не поворачиваясь от двух мониторов, подаёт голос мужчина с литровой кружкой без единой надписи.
...
>
> **Дмитрий:** С Павлом граница простая: всё, что внутри сервера, — его территория. Всё, что ходит к серверу с запросами, — наша. Где именно проходит эта граница, я буду показывать весь год. Ещё есть Евгений из маркетинга, этажом выше: он появляется со словами «маленькая доработка», и это ещё ни разу не было правдой.

Дмитрий разворачивает ноутбук. На экране — таблица заказов, первая строка подсвечена.

> **Дмитрий:** Смотри. Заказ номер один: Алиса Иванова, пятнадцатое января, девять ноль-ноль. Капучино и колд брю, статус paid. Первый заказ в истории Brew — Эмиль, наш основатель, до сих пор хранит бумажный чек в рамке. На этой строке я покажу тебе примерно всё: типы, джойны, транзакции, индексы.
>
> **Ты:** Целый курс — на одном заказе?
>
> **Дмитрий:** На одном заказе, одной базе и одном годе. Запомни два вопроса, с которых здесь начинается любой разбор: «покажи запрос» и «что база ответила». К концу года эти девять таблиц поедут потоком в соседнюю систему — и открывать этот поток будешь ты. А начнём с вопроса попроще: что вообще происходит на том конце сокета, когда приложение говорит с Postgres.
...
Данные, с которыми ты будешь жить весь курс, — те самые, из экскурсии Дмитрия: меню напитков (`drinks`), клиенты (`customers`), заказы (`orders`) и их позиции (`order_items`), кофейни (`shops`), остатки по точкам (`inventory`), блог (`articles`) и `outbox` — та самая таблица Ботыра, через которую события заказа уходят во внешний мир. Один и тот же набор таблиц проходит через весь курс: от первого подключения до финального капстона.

Якорь, который Дмитрий подсветил на экране, — **заказ #1** Алисы Ивановой — выбран не для красоты. К этой строке курс будет возвращаться снова и снова: на ней мы посмотрим, как работают JOIN'ы, что происходит при конкурентном обновлении, как читать план запроса. Когда дальше встретишь слово «заказ» и оно покажется слишком общим — вспоминай заказ #1.
