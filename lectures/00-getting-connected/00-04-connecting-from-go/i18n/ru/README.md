# 00-04 — Подключение из Go

Первая фича — твоя: поиск напитков по категории. Запрос собран склейкой строки: `"... WHERE category = '" + input + "'"`. На демо всё работало. На ревью Дмитрий молча вбивает в поле `' OR 1=1 --` и разворачивает экран. Витрина отдаёт **всё меню целиком** — в обход любого фильтра.

> **Ты:** Но на демо же работало!
>
> **Дмитрий:** Работало. В поле писали `coffee`, а не кавычку с хвостом. Твой ввод стал текстом запроса.
>
> **Ботыр:** У меня то же было в первый месяц. На тесте с десятью клиентами летало — а в проде в поле прилетает не только кофе. С тех пор всё внешнее едет только через `$1`, отдельно от текста запроса.
>
> **Дмитрий:** Разноса не будет. Разберём вместе, что уезжает на сервер.

Это SQL-инъекция — ошибка №1 всех списков веб-уязвимостей. Цель юнита — сделать первый запрос к Postgres из Go правильно: пул `pgxpool`, параметр `$1` — и на одном вводе увидеть разницу между склейкой и биндингом. Это raw-pgx юнит: мы намеренно пишем `rows.Scan` руками, чтобы в следующем юните (00-05) увидеть, что именно у нас заберёт sqlc.
...
`$1` — это **плейсхолдер**, а `"coffee"` едет отдельным аргументом. Ключевой момент в том, как это уходит на сервер: текст SQL и значения параметров передаются **раздельно**, разными полями протокола. Сервер разбирает SQL с плейсхолдерами отдельно от значений, а `$1` приходит отдельным полем и идёт к исполнителю как параметр — оно не вклеивается в текст запроса и вообще не проходит через парсер SQL. Значение `$1` физически не может стать частью команды.
...
```
   ...
   параметр $1 — ДВА конверта, значение едет мимо парсера:
     ...
     конверт 1 (текст):    "… WHERE category = $1"   ─▶  парсер ─▶ разобранный запрос с $1
     конверт 2 (значение): "coffee"  ────────────────────────▶ параметр для исполнителя
...
> [!NOTE]
> **Проверь себя.** Анти-демо подаёт в поле «категория» строку `' OR 1=1 --`.
> Предскажи: сколько строк вернёт небезопасная склейка и сколько — параметр `$1`?
> И почему именно столько в каждом случае?

> [!TIP]
> **Ответ.** Склейка — 5 строк, всё меню: ввод закрыл кавычку и дописал `OR 1=1`,
> условие стало всегда истинным, фильтр обойдён (как в выводе выше). Параметр `$1`
> — 0 строк: тот же текст ушёл отдельным полем мимо парсера и остался строковым
> значением категории `' OR 1=1 --`, которой в меню нет. Решает не «опасность»
> ввода, а то, попал он в текст запроса или в параметр.
