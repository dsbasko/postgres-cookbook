...
Первая фича — твоя: поиск напитков по категории. Запрос собран склейкой строки: `"... WHERE category = '" + input + "'"`. На демо всё работало. На ревью Марат молча вбивает в поле `' OR 1=1 --` и разворачивает экран. Витрина отдаёт **всё меню целиком** — в обход любого фильтра.

> **Ты:** Но на демо же работало!
>
> **Марат:** Работало. В поле писали `coffee`, а не кавычку с хвостом. Твой ввод стал текстом запроса.
>
> **Даня:** У меня то же было в первый месяц. На тесте с десятью клиентами летало — а в проде в поле прилетает не только кофе. С тех пор всё внешнее едет только через `$1`, отдельно от текста запроса.
>
> **Марат:** Разноса не будет. Разберём вместе, что уезжает на сервер.
