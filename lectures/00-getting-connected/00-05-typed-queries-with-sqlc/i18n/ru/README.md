...
> **Ботыр (в чате):** Переставил колонки в `SELECT` — тесты зелёные, а на витрине три напитка по имени coffee и категория «Латте». `Scan` промолчал: `name` и `category` — обе `text`, компилятору всё равно.
...
> **Ботыр:** Так это же ORM, только сбоку?
>
> **Дмитрий:** Нет. SQL остаётся твоим. Генерируется только обвязка.

Вердикт Дмитрия стоит развернуть. sqlc не прячет запросы за «магическим» API вроде `.Where("category", cat).First()` — ты по-прежнему пишешь `SELECT ... WHERE category = $1`, и именно этот SQL уезжает на сервер. sqlc убирает только механический разбор строк — тот самый цикл `Scan`, который мы писали в 00-04.
