...
Твоя задача как разработчика — **подобрать форму индекса под форму запроса**: узкое подмножество → частичный, «нужно только пару столбцов» → покрывающий, «должно быть уникально» → уникальный.
