...
Воркер Brew раз в секунду спрашивал «дай мне следующие необработанные заказы»: `WHERE status = 'pending' ORDER BY id LIMIT 100` — и полз. Ты несёшь на ревью миграцию: обычный индекс по `status`, чтобы база искала `pending` по индексу, а не сканом. Павел смотрит diff и отвечает одной строкой:

> **Павел (в ревью):** миллион done в индексе. зачем.

Заказов в статусе `pending` всегда мало — десятки из миллиона, остальное давно `done`. А индекс по `status` держал бы **все** строки, включая бесполезный миллион `done`: он был бы огромным, занимал бы память и медленно обновлялся на каждой смене статуса. Рядом — второй счёт: дашборд клиента тянул `SELECT customer_id, total ... WHERE customer_id = ?`, и на каждую строку индекс отправлял базу ещё и в саму таблицу за полем `total`, хотя по сути нужно было лишь два числа.
...
> **Ботыр:** А чего мелочиться — кладём в `INCLUDE` вообще все столбцы, и в таблицу больше никогда не ходим.
>
> **Дмитрий:** Ты только что раздул индекс обратно.
