...
Что мы упростили. Во-первых, мы выключили параллелизм и показали идеально селективный запрос (одна строка из миллиона) — на нём индекс выигрывает всегда. В жизни селективность бывает любой: запрос, возвращающий половину таблицы, планировщик **осознанно** пустит `Seq Scan`'ом, потому что читать пол-таблицы по индексу (прыжками по диску) дороже, чем подряд — и это правильное решение, а не «индекс не сработал». Во-вторых, числа из `ANALYZE` — это замер **одного** прогона на конкретной машине с конкретным состоянием кэша; «холодный» (первый после старта) и «горячий» прогоны дают разные `Buffers`/`time`, поэтому в проде на план смотрят несколько раз и сравнивают форму, а не отдельные миллисекунды. В-третьих, EXPLAIN отвечает на «как выполнился ЭТОТ запрос», а не «здорова ли база в целом»: системные вьюхи (`pg_stat_statements` — какие запросы суммарно съедают больше всего времени), автовакуум, раздувание таблиц, кэш-хит по всей базе — это уже приборная панель, которую держит твой DBA. Граница курса: твоя задача как разработчика — **уметь объяснить свой запрос и увидеть лишнюю работу в плане**; тюнинг сервера и мониторинг кластера — за её пределами.
