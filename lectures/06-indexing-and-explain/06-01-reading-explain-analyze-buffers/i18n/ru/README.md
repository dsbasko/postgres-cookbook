...
Вечерний пик. Дежурный по тикетам сегодня — ты, и первым в чат падает рапорт.

> **Руслан (в чате, 19:12):** Админка со статусами заказов висит. Открываю заказ — думает две-три секунды, к вечернему пику вообще колом. Обе точки.
>
> **Ты:** Запрос-то тривиальный — `SELECT * FROM events WHERE ref_no = ?`, одна строка на выходе. Почему две секунды?
>
> **Дмитрий:** Вопрос не что ты спрашиваешь, а как база отвечает. Покажи план.

Витрина открывалась мгновенно — пока таблица событий кассы была маленькой. К миллиону строк та же страница стала грузиться секундами: проблема не в запросе, а в том, **как** база его выполняла. Без индекса по `ref_no` ей приходилось читать и проверять каждую из миллиона строк, чтобы найти одну. Увидеть это можно одной командой — `EXPLAIN ANALYZE`.
