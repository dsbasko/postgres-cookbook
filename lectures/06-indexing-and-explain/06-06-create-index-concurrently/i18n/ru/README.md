...
Граница курса: твоя задача как разработчика — **знать, что индекс на горячую таблицу добавляют через `CONCURRENTLY`, а не обычным `CREATE INDEX`**, и помечать такие миграции соответствующе; оркестрация zero-downtime-выкаток — за её пределами.
