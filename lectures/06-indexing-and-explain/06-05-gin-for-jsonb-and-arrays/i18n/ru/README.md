...
В Brew у каждого напитка в админке висел блок «характеристики» — гибкий `jsonb` (`{"milk": "oat", "size": "L"}`) и массив тегов (`{coffee, seasonal}`). Пока меню было маленьким, фильтр «покажи всё с подарком» (`attrs @> '{"gift": true}'`) работал нормально. А потом с этажа маркетинга спустился Евгений:

> **Евгений:** Запускаю на главной «всё с подарком» — гость жмёт фильтр и сразу видит, что берут в подарок. Витрина готова, включаю к обеду.

На двухстах тысячах позиций фильтр встал: каждый запрос — полный `Seq Scan`. Ты пробуешь привычное `CREATE INDEX ON drink_specs (attrs)` — и `EXPLAIN` всё равно показывает `Seq Scan`. B-tree, который мы строили во всех прошлых юнитах, тут бесполезен в принципе: он индексирует значение **целиком**, а нам надо заглянуть **внутрь** — есть ли в jsonb такой ключ, есть ли в массиве такой элемент.
