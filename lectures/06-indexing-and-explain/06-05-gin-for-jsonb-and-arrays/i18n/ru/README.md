...
Что мы упростили. Во-первых, GIN — это про **поиск по содержимому** (`@>`, `?`, `&&`); если по jsonb-полю нужен поиск по равенству конкретного скаляра (`attrs->>'milk' = 'oat'`) — это снова обычный B-tree, но по **выражению** (`(attrs->>'milk')`, как в 06-03), а не GIN. Тип индекса выбирают под форму запроса, а не под тип столбца. Во-вторых, мы взяли селективный признак (0.5% строк) — на нём GIN явно выигрывает; на признаке, который есть у половины строк, планировщик осознанно вернётся к `Seq Scan` (тот же урок селективности, что в 06-01). В-третьих, у GIN своя цена записи и обслуживания (`fastupdate`, раздувание, перестройка) — это тюнинг под нагрузку, территория DBA. И в-четвёртых, jsonb — мощный, но не бесплатный инструмент моделирования: когда гибкая схема в jsonb оправдана, а когда поле стоит вынести в нормальный столбец с констрейнтом — отдельный разговор в 07-02. Здесь девелоперский вывод один: **для containment-поиска по jsonb/массиву ставь GIN и проверяй план в `EXPLAIN`**.
