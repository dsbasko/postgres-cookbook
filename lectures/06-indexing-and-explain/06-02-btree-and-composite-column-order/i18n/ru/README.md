...
Какие именно индексы нужны кластеру под реальную нагрузку, как ловить неиспользуемые (`pg_stat_user_indexes`) и дубликаты — это приборная панель твоего DBA; твоя задача как разработчика — **подобрать порядок столбцов под свои запросы и проверить его в `EXPLAIN`**.
