...
В Brew завели один индекс на таблицу позиций меню — по `(category, price)` — и закрыли тему «поиск по меню». Запрос «капучино дешевле 500» летал — пока с этажа маркетинга не спустился Евгений.

> **Евгений:** Люди смотрят на цену, а не на категорию. Дай к обеду отчёт: все позиции за 250 рублей, по всем категориям сразу. Индекс же есть — это быстро.

Отчёт неожиданно полз: тот же индекс, тот же столбец `price` внутри него — но запрос его будто не видел. Дело в том, что **порядок столбцов в составном индексе — не косметика**. B-tree хранит строки отсортированными сначала по первому столбцу, и только внутри одинаковых значений первого — по второму. Дмитрий берёт салфетку и рисует: спросить «по второму, не зная первого» — это как искать слово в словаре, отсортированном сперва по длине; знание второй буквы не помогает, пока не знаешь первую.
...
> **Ботыр:** В release notes PG18 — skip-scan для B-tree. Выходит, порядок столбцов теперь не важен, база сама разберётся?
>
> **Дмитрий:** Страховка, не проектирование.
>
> **Ботыр:** …то есть порядок всё равно на мне. Понял.
