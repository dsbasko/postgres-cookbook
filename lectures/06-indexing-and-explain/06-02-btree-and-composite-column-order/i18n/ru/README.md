...
Что мы упростили. Во-первых, мы поставили `category` первым **из-за демонстрации skip-scan** (нужна низкая кардинальность лидера), но в проде порядок столбцов выбирают под реальные запросы: первым — тот, по которому почти всегда фильтруют по равенству; диапазонный столбец (`price > X`, `created_at BETWEEN ...`) ставят последним, потому что после диапазона индекс «расходится» и следующие столбцы уже не сужают поиск. Во-вторых, skip-scan — приятная страховка, а не замена проектированию: при высокой кардинальности лидера он проигрывает `Seq Scan`, и полагаться на него вместо отдельного индекса по `(price)` не стоит. В-третьих, у составного индекса есть цена записи и места: каждый лишний индекс замедляет `INSERT`/`UPDATE` и занимает диск, поэтому «индекс на каждый случай» — это не бесплатно (про стоимость поддержки и `CREATE INDEX CONCURRENTLY` — в 06-06). Какие именно индексы нужны кластеру под реальную нагрузку, как ловить неиспользуемые (`pg_stat_user_indexes`) и дубликаты — это уже приборная панель твоего DBA; твоя задача как разработчика — **подобрать порядок столбцов под свои запросы и проверить его в `EXPLAIN`**.
