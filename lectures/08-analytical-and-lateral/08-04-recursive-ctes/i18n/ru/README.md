...
А через час прилетел второй инцидент. Логист собрал граф «куда везти партию дальше»: со склада — в цех, из цеха — в кафе. Кто-то по ошибке завёл строку, что из кафе остатки едут обратно на склад, — получилось кольцо `Склад → Цех → Кафе → Склад`. Ночная выгрузка маршрутов, которая обычно проходит за секунду, по этому кольцу ушла в бесконечный круг:

> **Нодыр (в чате, 23:50):** Выгрузка маршрутов висит. Обычно — секунда.
>
> **Павел (в чате):** вижу. запрос крутится по кругу. убил.

Павел находит повисший запрос в `pg_stat_activity`, среди активных запросов сервера, и снимает его руками: без защиты рекурсия по этому кольцу не остановится сама — так и будет ходить по кругу, пока сервер не упрётся в лимит. Оба инцидента — про одну и ту же конструкцию, рекурсивный `CTE`, которую мы пообещали ещё в `04-06`, когда впервые завели разговор о деревьях в реляционной модели. Пора её выдать.
