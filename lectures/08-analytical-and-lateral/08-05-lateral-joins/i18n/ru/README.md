...
Тикет от саппорта, 18:40: профиль клиента открывается секунду, дашборд
«топ-покупателей» на сотню клиентов — почти минуту, к вечеру пул соединений
захлёбывается. Масштаб: тысяча клиентов на экране — тысяча и один поход в
Postgres. Дежурный по тикету — ты.

За симптомом — паттерн `N+1`. Профиль клиента рисует блок «3 крупнейших
заказа»: бэкенд тянет список клиентов одним запросом, а потом в цикле, на
каждого клиента, шлёт в базу ещё один запрос за его тремя заказами. Половина
латентности уходит просто на сетевые round-trip между сервисом и базой.
Хотелось другого: один запрос, который сам, внутри базы, посчитает топ-3 сразу
для всех клиентов.
...
Пока top-N собран, к твоему столу подходит Евгений — телефон экраном к тебе, на
экране профиль клиентки.

> **Евгений:** Опять Карина?! Была в списке — а теперь пропала из профилей.
>
> **Ты:** `CROSS` выбрасывает тех, у кого правый подзапрос пуст?

Ровно об этом — следующий разрез.
