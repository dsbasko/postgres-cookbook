...
`row_number()` недетерминирован без *полного* `ORDER BY`. Если двум строкам нечем различиться, движок волен пронумеровать их в любом порядке — и на следующем прогоне порядок может смениться. Поэтому в `wu` стоит `drink` вторым ключом: он гарантирует, что `1, 2, 3, 4` каждый раз лягут одинаково. В проде твой аналитик добавит такой tie-break всегда, когда ранг используется для отбора строк, иначе «топ-1» может тихо выбирать разные строки между запусками.

С `rank()` и `dense_rank()` ровно наоборот: лишний tie-break *ломает* ничью. Стоит дописать в их `ORDER BY` уникальную колонку — и 120/120 перестанут быть пирами, а функции выродятся в `row_number`. Именно поэтому в нашем запросе для них отдельное окно `wt` *без* `drink`. Решая «по чему ранг», думай, что должно считаться ничьей, и кладёшь в `ORDER BY` ровно эти колонки — ни одной лишней.

`ntile()` при неделимом числе строк не падает, а раскидывает остаток по *первым* корзинам: восемь строк на четыре корзины делятся ровно, а вот девять дали бы корзины `3, 2, 2, 2`. Знай об этом, когда строишь децили на данных, чьё число не кратно числу корзин — первые корзины окажутся чуть полнее.

«Top-N на группу» через `row_number() = 1` в CTE — рабочая классика, но не единственный путь. На огромных партициях иногда быстрее `LATERAL` (к нему придём в 08-05) или индекс под `(category, units DESC)`, по которому top-1 каждой группы достаётся почти даром. В проде твой DBA посмотрит на план: если `row_number` гонит сортировку всей таблицы ради одной строки на категорию, индекс или `LATERAL` срежут работу на порядок.

И последнее — выбор между `rank` и `dense_rank` это не про производительность, а про смысл отчёта: нужны ли в нумерации «дырки» после ничьих. Хочешь «после двух серебряных сразу четвёртое» — `rank`. Хочешь «уровни без пропусков, второй-третий-четвёртый» — `dense_rank`. Решай по тому, как отчёт будут читать люди.
