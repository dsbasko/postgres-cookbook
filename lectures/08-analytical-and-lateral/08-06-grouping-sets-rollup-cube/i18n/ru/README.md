...
Что мы упростили. Главная ловушка — `NULL` в строке-подытоге **неотличим** от настоящего `NULL` в данных, если колонка их допускает: без `grouping()` ты не скажешь, `NULL` тут — это «итог по всем категориям» или «у строки реально не было категории». Как только в группируемой колонке возможны реальные `NULL`, метка `grouping()` (и/или `coalesce` для подписи) перестаёт быть украшением и становится обязательной. Дальше: `CUBE` растёт как `2^N` комбинаций по числу колонок — на двух измерениях это четыре группировки, на пяти уже тридцать две; на многих измерениях `CUBE` дорог, и под конкретный дашборд лучше брать `GROUPING SETS` с ровно нужными срезами. И главное: это разовая сводка «на лету» по небольшой таблице фактов, а не замена настоящему OLAP-кубу или материализованным агрегатам на больших данных. Когда срезов много, данные тяжёлые, а витрину дёргают часто, построение и регулярное обновление такой витрины (предрасчёт, материализованные представления, отдельная аналитическая база) — это уже забота аналитической платформы и твоего DBA, а не одного `SELECT` в продовом OLTP.
