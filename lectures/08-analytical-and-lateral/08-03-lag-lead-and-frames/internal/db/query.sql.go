...
const dayOverDay = `-- name: DayOverDay :many
...
func (q *Queries) DayOverDay(ctx context.Context) ([]DayOverDayRow, error) {
	rows, err := q.db.Query(ctx, dayOverDay)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const movingAverage = `-- name: MovingAverage :many
SELECT
    day::text AS day,
    cents,
...
type MovingAverageRow struct {
	Day     string `json:"day"`
	Cents   int64  `json:"cents"`
	MaRows  string `json:"ma_rows"`
	MaRange string `json:"ma_range"`
}
...
func (q *Queries) MovingAverage(ctx context.Context) ([]MovingAverageRow, error) {
	rows, err := q.db.Query(ctx, movingAverage)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []MovingAverageRow{}
	for rows.Next() {
		var i MovingAverageRow
		if err := rows.Scan(
			&i.Day,
			&i.Cents,
			&i.MaRows,
