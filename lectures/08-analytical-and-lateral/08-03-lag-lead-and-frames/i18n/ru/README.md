...
`schema.sql` заводит лабораторную таблицу `daily_revenue_lab` (дата + центы) с намеренным пропуском 5 февраля — канон Brew с тремя заказами для ровного временного ряда не годится, поэтому таблица своя, а канон не трогаем. `cmd/demo/main.go` — тонкий: открывает пул, вызывает `DayOverDay` и `MovingAverage`, печатает обе таблицы через `tabwriter`. Среднее завёрнуто в `round(..., 2)`, поэтому текст детерминирован и совпадает с тем, что вставлено в `## Запуск` ниже.
...
`ROWS` считает строки и слеп к смыслу промежутков между ними: для него «две строки назад» — это две строки назад, даже если между ними провал в неделю. `RANGE` считает по значению `ORDER BY` и потому корректен на дырявом ряде, но платит за это: ему нужен сортируемый тип в `ORDER BY` (дата, число, timestamp — не любой), и работает он дороже, потому что для каждой строки определяет границы окна по значению, а не по счётчику строк. Практическое правило: «скользящее среднее за календарные N дней» в финансах и аналитике почти всегда хотят именно как `RANGE` по дате — чтобы выходные и пропуски не раздували окно лишними строками; `ROWS` уместен там, где важна именно позиция (последние 3 события, что бы ни стояло в датах).

Отдельный сюрприз новичка — фрейм по умолчанию. Если в `OVER (...)` есть `ORDER BY`, но фрейм не выписан явно, Postgres подставляет `RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`. Поэтому `sum(...) OVER (ORDER BY ...)` из 08-01 уже давал running total, хотя мы про фреймы тогда не говорили — это и был дефолтный фрейм «от начала окна до текущей строки». Привыкни проверять: если агрегат поверх `ORDER BY` ведёт себя как накопительный, а ты этого не закладывал — почти наверняка сработал умолчательный `RANGE UNBOUNDED PRECEDING`.

В проде твой DBA смотрит на фрейм как на цену: на больших рядах оконный агрегат — это сортировка по `ORDER BY` плюс буфер строк под фрейм, и эта цена видна в плане (`EXPLAIN`, модуль 06) отдельным узлом `WindowAgg`. Память под этот буфер и решение, держать ли его в `work_mem` или сливать на диск, — уже его забота, а не запроса.
