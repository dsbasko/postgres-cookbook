...
В Brew есть фоновая работа: разослать чеки, пересчитать остатки, отправить пуш
«ваш заказ готов». Всё это складывается в таблицу-очередь, которую параллельно
разбирают воркеры.

> **Евгений:** Алисе Ивановой — два пуша «заказ готов» и два чека на почту.
> Заказ один. Она уже пишет в поддержку.
>
> **Дмитрий:** Покажи воркера.

Ты показываешь: воркер берёт `SELECT`'ом самую старую задачу в статусе `queued`,
потом переводит её в `processing` через `UPDATE`. Дмитрий кивает — два воркера
читают одну и ту же строку в один и тот же момент, оба берут её в работу, отсюда
и два пуша.

> **Ты:** А если залочить строку прямо при чтении?

Это `FOR UPDATE` из 05-03 — только он ломает ровно то, ради чего очередь и
заводили.

> **Дмитрий:** Воркеры встанут паровозиком: второй ждёт, пока первый закоммитит.
> Параллелизма нет — N воркеров работают как один. Очереди нужно другое: пропусти
> занятое, не жди.
...
> **Павел (в чате):** транзакцию воркера держи короткой. взял, сменил статус,
> закоммить. тяжёлое — снаружи.
