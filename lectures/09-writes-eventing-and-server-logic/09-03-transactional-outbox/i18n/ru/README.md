...
«хотя бы один раз» — да (consumer на другой стороне глушит дубли по
`outbox_id`, для этого в каноне есть `processed_outbox_ids`).
...
Outbox честно решает атомарность факта и события, но за гарантию доставки ты
по-прежнему отвечаешь сам. relay даёт **at-least-once**, не exactly-once: если
он упал между `publish` и `UPDATE published_at`, после перезапуска событие
уедет повторно. Поэтому на стороне consumer'а нужна **идемпотентность** — гасить
дубли по ключу (в каноне для этого есть `processed_outbox_ids`). Держи
транзакцию записи короткой, а саму отправку в брокер делай в relay вне
транзакции чтения — иначе медленная сеть до брокера будет держать блокировки и
горизонт видимости (см. 05-02 и заборчик 09-02).

Таблица `outbox` постоянно растёт и сразу же чистится — это типичная
«high-churn» таблица, источник bloat. В проде её надо периодически подчищать
(удалять давно опубликованные строки) и не забывать про autovacuum — но это уже
эксплуатация, территория твоего DBA, мы её здесь не трогаем.

И развилка: relay можно написать руками (как тут — читаем `outbox` и публикуем),
а можно вообще не писать своего relay, а отдать `outbox`/канон в **логическую
репликацию** и снять события через CDC (Debezium). Второй путь — это и есть
capstone 10-05: `REPLICA IDENTITY FULL` на CDC-источниках + `CREATE PUBLICATION`,
и Debezium из `kafka-cookbook` читает наши таблицы без переписывания схемы.
