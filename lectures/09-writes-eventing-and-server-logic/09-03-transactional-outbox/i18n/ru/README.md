...
Таблицы `orders` и `outbox` здесь — **канонические**, из `schema/brew.sql`,
...
своих лабораторных столов, а работает на настоящем каноне.
...
(он есть в каноне) — relay не сканирует всю историю outbox, только хвост
...
репликацию, без таблицы-события и без своего relay; Debezium читает канон
напрямую). Это не два шага одного процесса, а две разные точки входа — выбираешь
одну.
...
- **Развилка relay против CDC.** relay можно написать руками (как тут — читаем
  `outbox` и публикуем), а можно вообще не писать своего relay: отдать канон в
...
доставленными. Это at-least-once: consumer обязан быть идемпотентным. Канонические
