...
все довольны — до первой же ночи под нагрузкой.

pgbouncer выкатили в 03:11. Через минуту посыпались алерты: экран заказов молчит,
advisory-лок повис, параметрический запрос валится с ошибкой про отсутствующий
prepared statement. Годами работало без единого нарекания — и встало разом. Ночью
дежуришь ты, впервые. В 03:14 пишешь Павлу.

> **Ты (в чате, 03:14):** Лок висит. LISTEN молчит. prepared падает. Что общего?

Ответ — сразу, фирменными строчными.

> **Павел (в чате):** пул. транзакционный. сессии больше нет.

Причина одна, и её надо понять буквально дословно. Транзакционный пул держит
реальный бэкенд за тобой ровно на **одну транзакцию**, а не на «сессию».
Закрываешь транзакцию — бэкенд возвращается в общий котёл, и следующую транзакцию
пул может отдать тебе уже на **другом** бэкенде. Всё, что живёт на уровне сессии,
а не транзакции, при такой пересадке молча ломается.
...
Утром — ретро инцидента у доски.

> **Дмитрий:** Три вещи легли разом. Назови их — и почему каждая.
>
> **Ты:** Advisory-лок, LISTEN, prepared statement. Все три — на сессии.
>
> **Павел:** Пул даёт бэкенд на транзакцию, не на сессию. Что на сессии — то
> теперь не твоё.
>
> **Дмитрий:** Значит, чиним не пул — чиним три места в коде. Каждое
> session-scoped на транзакционный эквивалент. Ниже — картинка пересадки и
> таблица замен.
