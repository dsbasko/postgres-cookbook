...
non-sargable. N+1 крутится на **канонических** `customers`/`orders` из `schema/brew.sql`.
...
Это всё **смолы на стороне приложения**, а не работа DBA: база здорова, лечится код.
Перечислим оговорки.

Keyset быстр **под подходящим индексом** и при сортировке по нему же, но ценой
гибкости: он не умеет прыгать на произвольную страницу («покажи сразу 4000-ю») — он
идёт от границы предыдущей. Если продукту нужна именно нумерованная навигация по
страницам, keyset не подойдёт; механика keyset подробно разобрана в 03-02, а индексы и
чтение `EXPLAIN` — в модуле 06.

Лечение N+1 здесь — `= ANY`, но это не единственный путь: тот же список с заказами
часто естественнее собрать одним `JOIN` (модуль 04) или `LATERAL`-подзапросом «топ-N на
клиента» (08-05) — выбор зависит от того, что именно нужно вернуть.

`= ANY($1)` чинит не только распухание кэша планов: один параметр-массив заодно обходит
...
запросе `lower`, а индекс по сырому `email` — он бесполезен; подробнее про это в 06-03.

`SELECT *` ломается не только сетью: код, завязанный на «все столбцы», падает или начинает
возить мусор, когда колонки **добавили или поменяли местами**. Явный список столбцов —
