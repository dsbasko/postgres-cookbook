...
К твоему столу Евгений приводит двоих из соседней команды `kafka-cookbook`.
Им нужен живой поток изменений Brew: цену правит
бариста, статью публикует маркетинг, у клиента меняется телефон — всё это
должно доезжать до их мира, где тот же Brew живёт как события в Kafka.

> **Евгений:** Нужны наши изменения — меню, блог, клиенты. Живым потоком, не
> выгрузкой раз в сутки. Я обещал, что отдадим сегодня.

Гости ждут молча — межкомандный контракт, лишних слов нет. Олег крутится на
стуле рядом.

> **Олег:** Стоп, а почему не outbox с relay, как раньше?
>
> **Ты:** outbox — это наша доставка, руками. Здесь её берёт на себя Postgres:
> изменение уже лежит в журнале.

Ты открываешь `PUBLICATION` на трёх таблицах — `drinks`, `articles`,
`customers` — и показываешь Олегу: WAL и так хранит каждое изменение, Debezium
вычитывает его без нашего кода. Гость кивает на свою `init.sql` — колонки те же.

> **Ты:** Переименуем у себя хоть одну колонку — сломаем им Debezium.

Это и есть Change Data Capture: не свой relay (путь 09-03) и не триггеры, а
логическая репликация Postgres. Так Postgres становится источником истины, а
WAL — нашей шиной событий.
...
эстафету, Kafka её принимает.

Публикация открыта, слот проверен — и в опенспейсе тихо собирается вся команда.
Павел кладёт рядом с твоей клавиатурой потрёпанный блокнот инцидентов, открытый
на чистой странице.

> **Павел:** Слот твой. Следи за диском.

Он не объясняет, чем грозит забытый слот, — это ты уже знаешь. Эмиль спускается
сверху и ставит на стол рамку: бумажный чек заказа #1 — Алиса Иванова, капучино
и колд брю, пятнадцатое января.

> **Эмиль:** Первый заказ уезжает в большой мир. Проследи, чтобы доехал.

Олег подкатывается на стуле, экран горит красным вопросом.

> **Олег:** Стоп, а почему поток у нас встал, а на той стороне порядок?
>
> **Ты:** Покажи запрос.

Он показывает. И на секунду в комнате двое: ты, который спрашивает сейчас, — и
Олег, копия тебя годичной давности. Дмитрий стоит за спиной, дорисовывает
что-то на кофейной салфетке и кладёт её поверх блокнота Павла.

> **Дмитрий:** Год на одной салфетке: покажи запрос, слушай, что ответит база,
> инвариант — в схему, доставку не пиши руками. Остальное — детали.

Это был последний юнит. Спасибо тебе за этот год в Brew.
