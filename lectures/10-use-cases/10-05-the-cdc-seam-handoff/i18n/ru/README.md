...
В CDC-эстафету едут три канонические таблицы: `drinks` (меню), `articles` (блог),
`customers` (справочник клиентов). На них в каноне уже стоит `REPLICA IDENTITY
...
`internal/db/`. Демо последовательно: накатывает канон, применяет артефакт
...
Логический слот репликации, который **никто не вычитывает**, держит WAL: Postgres
не может удалить сегменты журнала, пока их не подтвердил самый отстающий
потребитель — и диск медленно заполняется. Наше демо честно сносит слот в конце,
но в проде застрявший слот (умерший Debezium, отключённый consumer) — реальный
путь к `No space left on device`. За слотами надо следить (`pg_replication_slots`)
и подчищать мёртвые — это территория твоего DBA, мы её здесь не трогаем.

`test_decoding` — отладочный плагин, а не то, чем читает Debezium: он печатает
...
Каждый `UPDATE`/`DELETE` теперь пишет в журнал всю старую строку, а не один PK —
на горячей таблице с широкими строками это заметный рост объёма WAL и нагрузки на
...
И сам сквозной пайплайн — `Debezium → Kafka → sinks` — мы здесь **не запускаем**.
Это уже сторона `kafka-cookbook`: следующий курс. Наша задача — отдать корректный
поток, и она выполнена.
...
Финальный кадр — правило байт-совместимости канона, которое мы держали с первого
...
схемы. Переименуй здесь хоть одну колонку канона — и эстафета рвётся.
