...
Меню Brew в приложении показывают постранично: «ещё 20 напитков», «следующая страница». Пока страниц мало — всё работает. А сегодня Евгений спускается с телефоном экраном вперёд — на нём жалоба гостя из приложения.

> **Евгений:** Гость долистал меню до пятисотой страницы — и приложение задумалось. Почему низ меню медленнее верха, кофе-то тот же?

Открываешь запрос: страница 500 — это `OFFSET 10000`, и тормозит именно он, хотя отдаёт всё те же 20 строк. Причина не в количестве отданного, а в количестве пройденного: `OFFSET` заставляет сервер вычислить и **отбросить** первые 10000 строк, прежде чем вернуть нужные.
...
> **Ботыр:** Каюсь: я всю жизнь беру LIMIT/OFFSET, а глубокие страницы кэширую. На тесте летало.
>
> **Дмитрий:** Посчитай, сколько строк база прочитала ради страницы пятьсот.

Считать удобнее на картинке — разложим оба способа по строкам.
