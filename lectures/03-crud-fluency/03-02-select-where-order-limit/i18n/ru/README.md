...
На пяти строках seed'а разницы в скорости нет — её видно на больших таблицах и под нужный индекс. Что мы упростили: keyset-пагинация быстра ровно тогда, когда `ORDER BY` лёг на индекс — здесь это был бы индекс `(base_price, id)`; без него сервер всё равно отсортирует всю таблицу, и преимущество испарится (про индексы и чтение планов — модуль 06). Мы также не показали выдачу «общего числа страниц»: `count(*)` по большому отфильтрованному набору сам по себе дорог, и в проде его либо кэшируют, либо заменяют на «есть ли ещё» (запросить `LIMIT n+1` и проверить, пришла ли `n+1`-я строка). И `OFFSET` не «плохой» — для первых нескольких страниц небольшого набора он проще и совершенно нормален; keyset нужен там, где листают глубоко. В проде выбор пагинации зависит от того, нужны ли прыжки на произвольную страницу (тогда без `OFFSET`/нумерации не обойтись) или достаточно «показать ещё» (тогда keyset).
