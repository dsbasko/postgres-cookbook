...
Здесь мы откатываем катастрофу **сами**, потому что заранее знаем, что она случится. В проде ты заметишь забытый `WHERE` не всегда — поэтому полагаются не на бдительность, а на барьеры: ревью миграций и запись-скриптов, прогон на staging, а для интерактивного `psql` — режим, где транзакция не коммитится автоматически (в `psql` это `\set AUTOCOMMIT off`, и тогда каждый `UPDATE`/`DELETE` ждёт явного `COMMIT`). Что мы упростили: `price_lab` крошечная, и `UPDATE`/`DELETE` по всей таблице мгновенны; на большой таблице массовая запись — это ещё и долгая блокировка строк (другие транзакции ждут) и распухание (`UPDATE` в MVCC создаёт новые версии строк), о чём — модуль 05 и `VACUUM`. И `RETURNING` на массовом `UPDATE` притащит все изменённые строки в приложение — на миллионе строк это много трафика; тогда берут `:execrows` (только число) или правят пачками. В проде опасные `DELETE` часто заменяют на «мягкое удаление» (`deleted_at`-флаг), чтобы данные можно было вернуть.
