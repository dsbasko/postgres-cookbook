...
`RETURNING old/new` отдаёт «до и после» только для тех строк, которые **эта команда** трогает, и только в её транзакции — это не журнал изменений. Когда историю надо хранить независимо от того, какой код сделал запись (и для `UPDATE` из любого места, и для прямых правок в `psql`), берут триггер `AFTER` с `OLD`/`NEW`, пишущий в audit-таблицу — это модуль **09-05**, и там же мы обсудим, когда логику стоит держать в БД, а когда нет. Что мы упростили: `old`/`new` — фича именно **PG18**; на более старых версиях её нет, и «до-значение» добывают `SELECT`-ом до `UPDATE` (с риском гонки) или триггером. И ещё практический момент: `old`/`new` пока не понимает наш кодогенератор (sqlc v1.30.0) — поэтому в проде с sqlc такой запрос придётся либо писать «сырым» через `pgx` (как здесь), либо ждать поддержки в инструменте.
