...
Утро начинается с двух ночных сообщений Нодыра.

> **Нодыр (в чате, 23:50):** Ночной синк остатков упал. duplicate key.
>
> **Нодыр (в чате, 23:57):** Перезапустил — прошло. Файл как файл, что не так-то?

«Перезапустил — прошло» — худший вид бага: он не воспроизводится, а значит, никуда не делся. Ночной синк берёт выгрузку остатков `(кофейня, напиток, остаток)` из учётной системы; часть пар уже есть в базе — их надо обновить, часть новые — их надо вставить. Наивный код для каждой строки сначала делает `SELECT` и по результату решает: `INSERT` или `UPDATE`. Это три запроса там, где хватит одного, и — что хуже — гонка: той ночью повторная выгрузка из учётной системы наложилась на первый прогон, и между `SELECT` и `INSERT` одного экземпляра синка ту же пару успел вставить второй. `INSERT` упал на нарушении уникальности — тот самый `duplicate key`, SQLSTATE `23505` из модуля 02. Ручной перезапуск шёл уже в одиночку, без гонки — вот и «прошло».
