...
Программа лояльности Brew дорастает от регистраций до карт. Евгений спускается в опенспейс с телефоном экраном вперёд: на нём макет кассового экрана с пустым полем для номера карты.

> **Евгений:** В понедельник выдаём первую карту — Алисе Ивановой. Наш заказ номер один, ей и карта номер один. Требование одно: номер сразу на экран. Гость стоит у кассы и ждать не будет.
>
> **Ты:** `id` карте раздаёт база. Вставить строку, а потом прочитать её обратно `SELECT`-ом — и номер на экране?
>
> **Дмитрий:** Это два запроса. Что происходит между твоим `INSERT` и твоим `SELECT`?
