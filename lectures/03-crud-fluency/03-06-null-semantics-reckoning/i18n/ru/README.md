...
> **Анна (в чате, 09:20):** Приложение. Меню пустое. 09:20. Гости заказывают у стойки.
...
>
> **Марат:** Запрос честный. Второй вопрос: что база ответила?
>
> **Ты:** Ноль строк. Ни одной ошибки — это и пугает.

След приводит в `unavailable`: вчера стоп-лист правили руками, и в одной строке вместо id напитка остался `NULL`. Правка была нужной — Стас снимал с продажи сезонный раф. Марат пишет на салфетке: `true AND NULL = NULL`.

> **Стас:** Тот самый NULL, который ест моих клиентов? Теперь он взялся за меню?
>
> **Марат:** Он никого не ест. База на каждый напиток ответила «не знаю», а «не знаю» для WHERE — это «нет».
...
> **Даня:** Подожди, но NULL = NULL — это же true: слева «не знаю», справа «не знаю». Одинаковые же.
>
> **Марат:** NULL — не значение, а «не знаю». «Не знаю» равно «не знаю»?
>
> **Даня:** …Не знаю.
>
> **Марат:** База отвечает так же.
...
Это финал модуля 03 — «CRUD-беглость». Вечером Стас дочитывает постмортем стоп-листа — и задерживается у твоего стола.

> **Стас:** NULL я запомнил. Теперь скажи: а сколько всего таких, как Карина, — зарегистрировались и молчат?
