...
Утро начинается с рапорта в командном чате.

> **Руслан (в чате, 09:20):** Приложение. Меню пустое. 09:20. Гости заказывают у стойки.

Запрос «напитки не в стоп-листе» — `... WHERE id NOT IN (SELECT drink_id FROM unavailable)` — вернул **ноль** напитков, будто недоступно всё меню. Ни ошибки, ни предупреждения: логи чистые.

> **Дмитрий:** Покажи запрос.
>
> **Ты:** Вот. Он год работал — что сломалось за ночь?
>
> **Дмитрий:** Запрос честный. Второй вопрос: что база ответила?
>
> **Ты:** Ноль строк. Ни одной ошибки — это и пугает.

След приводит в `unavailable`: вчера стоп-лист правили руками, и в одной строке вместо id напитка остался `NULL`. Правка была нужной — Евгений снимал с продажи сезонный раф. Дмитрий пишет на салфетке: `true AND NULL = NULL`.

> **Евгений:** Тот самый NULL, который ест моих клиентов? Теперь он взялся за меню?
>
> **Дмитрий:** Он никого не ест. База на каждый напиток ответила «не знаю», а «не знаю» для WHERE — это «нет».

Вот и расплата за тизер из 01-02: `NULL` — не «пусто», а «неизвестно». Цель юнита — разобрать эту трёхзначную логику до конца и больше в неё не попадать.
...
> **Ботыр:** Подожди, но NULL = NULL — это же true: слева «не знаю», справа «не знаю». Одинаковые же.
>
> **Дмитрий:** NULL — не значение, а «не знаю». «Не знаю» равно «не знаю»?
>
> **Ботыр:** …Не знаю.
>
> **Дмитрий:** База отвечает так же.
...
Это финал модуля 03 — «CRUD-беглость». Вечером Евгений дочитывает постмортем стоп-листа — и задерживается у твоего стола.

> **Евгений:** NULL я запомнил. Теперь скажи: а сколько всего таких, как Карина, — зарегистрировались и молчат?

Ответить одним честным числом пока нечем — для этого надо связывать таблицы. Дальше — модуль **04 «Запросы по таблицам»**: связываем данные `JOIN`'ами, агрегируем через `GROUP BY`/`HAVING`, берём «последнее на клиента» через `DISTINCT ON`, и снова встречаем ловушку `NOT IN` + `NULL` — теперь уже как часть выбора между `EXISTS` и `IN`.
